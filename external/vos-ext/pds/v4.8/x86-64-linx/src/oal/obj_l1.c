...
ODLTREE current_node, save_node = NULL;
...
if (OaKwdValuetoStr( "ODL_VERSION_ID", root_node, &str) == 0) {
  if (strcmp( str, "ODL3") == 0)
    return( root_node);
}
...
char *buf, *str, *label_filename, *data_filename, tmp_filename[128];
...
switch( record_type) {
  ...
  case OA_VARIABLE_LENGTH:
	...
	if((label_lines > 0) && 
	   (OdlFindKwd(root_node, "LABEL_RECORDS", "*", (unsigned long) 0, (unsigned short) ODL_THIS_OBJECT) != NULL)) {
		OaLongtoKwdValue("LABEL_RECORDS", root_node, label_lines);
	}
  break;
  ...
  case OA_FIXED_LENGTH:
	...
	if((label_records > 0) && 
	  (OdlFindKwd(root_node, "LABEL_RECORDS", "*", (unsigned long) 0, (unsigned short) ODL_THIS_OBJECT) != NULL)) {
		OaLongtoKwdValue( "LABEL_RECORDS", root_node, label_records);
	}
  break;
}
...
for (i=0; i<kwds; i++) {
  sprintf( buf, "%-ld", (pointer_info[i].offset+1));
  if ((record_type == OA_STREAM) || (record_type == OA_UNDEFINED_RECORD_TYPE))
    strcat( buf, " <BYTES>");
  OaStrtoKwdValue( OdlGetKwdName( pointer_info[i].kwdptr), 
                   (pointer_info[i].kwdptr)->parent, buf);
...
char *kwd_value_str, *ptr, compressed_filename[80], encoding_type[80], *user_home;
...
if (kwd_value_str == NULL) {
  ...
  OaReportError( error_string);
           ...
           (strstr( kwd_value_str, "ODL3") == NULL)) {
  ...
  if (OdlExtractFilename( *data_filename) == *data_filename) {  /* no directory */
    ...
    if (ptr != NULL) {  /* ptr is the name of the data file proven to exist.  */
      ...
      *data_filename = ptr;
      ...
      /* If we have a JP2 compressed file, then we should search the user's
	     home directory for the uncompressed image file.
	  */
	  OaKwdValuetoStr( "ENCODING_TYPE", next_node, &ptr);
	  if(ptr != NULL)
	  {
          strcpy(encoding_type, ptr);
	      StripLeading(encoding_type, '"');
	      StripTrailing(encoding_type, '"');

 	      if(strcmp(encoding_type, "JP2") == 0)
		  {
			  char dir[256]={0};
#ifdef MSDOS_TC
...
#endif
			  ptr = OdlCheckFileSpec(user_home, *data_filename);
			  if(ptr != NULL)
			  {
                  LemmeGo(*data_filename);
                  *data_filename = ptr;
