  ...
  if ((start_line != (image_handle->next_line - 1)) || 
    ...
    while (bytes_to_read > 0) {
      ...
      if (OalReadStream( image_handle_object->stream_id, bytes_to_read, 
		...
		{
			xvt_dm_post_error
				("Bytes read: %d.\nBytes attempted to be read: %d\nline = %d, sample = %d", bytes_read, bytes_to_read, start_line, start_sample);
		}
#endif
        return(0);
      }
...
if (band_storage_type == OA_SAMPLE_INTERLEAVED) {
  double suffixSizeFactor = 4 / core_item_bytes;
...
/* total_bytes_to_read = record_bytes * line_samples *lines - (band -1) * core_item_bytes; *//**DWS     1/10/06**/
  total_bytes_to_read = core_item_bytes * (core_items[0] + suffix_items[0]*suffixSizeFactor)
	         * (core_items[1] + suffix_items[1]*suffixSizeFactor)
			 * (core_items[2] + suffix_items[2]*suffixSizeFactor);
...
OA_OBJECT OaReadSpectrumFromQube( input_node, line, sample, int band)
ODLTREE input_node;
int     line;
int     sample;
int     band;
...
OA_OBJECT OaReadSpectrumFromQube( ODLTREE input_node, int line, int sample, int band)
...
/*line, sample*/
if (band_storage_type == OA_SAMPLE_INTERLEAVED) {
  long sample_size = 0, full_band_size = 0;
  line_samples = core_items[1];
  lines = core_items[2];
  line_suffix_bytes = suffix_items[0]* 4;
  skip_bytes = 0;
  ...
  full_band_size = (bands * core_item_bytes) + (suffix_items[0] * 4);
  sample_size = (suffix_items[1] * (bands + suffix_items[0]) * 4);

  file_offset += (line - 1) * ((line_samples * full_band_size) + sample_size);
  file_offset += (sample - 1) * full_band_size;
  file_offset += (band - 1) * core_item_bytes;
...
{
	if (OalReadStream( stream_id, (long) line_samples * core_item_bytes, &buf, file_offset, &bytes_read) != 0) 
	{
		sprintf( error_string, "%s: OalReadStream returned error, aborting.",
																       proc_name);
		OaReportError( error_string);
...
*/
  ...
  {
	 if (OalReadStream( stream_id, (long) line_samples * core_item_bytes, &buf, (long) -1, &bytes_read) != 0) 
	 {
       if (total_bytes_read < total_bytes_to_read) 
	   {
          sprintf( error_string, "%s: OalReadStream returned error, aborting.",
...
int object_class=0, object_interchange_format = 0;
char kwd_name[80]={0}, kwd_value[40]={0}, tmp_file_name[80]={0}, *str = NULL;
PTR ptr = NULL;
FILE *tmp_fd = NULL;
long bytes_written=0, bytes_to_write=0, bytes_remaining=0, byte_count=0;
long object_start_position=0, record_count=0;
ODLTREE current_node = NULL, root_node = NULL;
KEYWORD *kwd_ptr = NULL, *next_kwd_ptr = NULL;
