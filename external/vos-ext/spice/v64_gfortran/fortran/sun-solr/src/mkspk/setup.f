...
C        then the error 'SPICE(NOINPUTFILENAME)' will be signalled.
C        
C     2) If nput file name is blank, then the error
C        'SPICE(BLANKINPUTFILENAME)' will be signalled.
C        
C     3) If nput file does not exist, then the error
C        'SPICE(INPUTFILENOTEXIST)' will be signalled.
C        
C     4) If output SPK file name was not provided neither on
C        the command line nor as a value of the setup file keyword,
C        then the error 'SPICE(NOOUTPUTFILENAME)' will be signalled.
C        
C     5) If output file name is blank, then the error
C        'SPICE(BLANKOUTPTFILENAME)' will be signalled.
C        
C     6) If append flag specified in the setup file keyword 
C        is not recognizedthe the error 'SPICE(UNRECOGNAPPFLAG)'
C        will be signalled.
C        
...
C        is not recongnized, then the error 'SPICE(UNRECOGNDATATYPE)'
C        will be signalled.
C        
...
C        'SPICE(NOFRAMENAME)' will be signalled.
C        
...
C        'SPICE(NOLINESPERRECCOUNT)' will be signalled.
C        
...
C        'SPICE(BADLINEPERRECCOUNT)' will be signalled.
C        
...
C        keyword is not recongnized, then the error
C        'SPICE(UNRECOGNPRECTYPE)' will be signalled.
C        
...
C        'SPICE(NOTAPCKFILE)' will be signalled.
C        
C     31) If input flag is not recognized, then the error
C        'SPICE(MKSPKBUGSETUP1)', 'SPICE(MKSPKBUGSETUP2)',
C        'SPICE(MKSPKBUGSETUP3)' or 'SPICE(MKSPKBUGSETUP4)' 
C        will be signalled.
C
C$ Files
C
...
C
C
C     Input and output filenames length.
C
      INTEGER               FNMLEN
...
C
C        Retrive TIME_WRAPPER value. Parameter KTIMWR declared in 
C        include file. 
C
...
C
C        Retrive DATA_ORDER value. Parameter KDATOR declared
C        in include file.  
C
...
C
C        Retrive DATA_DELIMITER value and check it. Parameter KDATDL
...
C        
     ...
     .                    'file keyword ''#'' is not recongnized. '  //
...
C$ Detailed_Input
C
C     FLAG        is the string containig text identifier for the 
C                 keyword of interest.
C                
C$ Detailed_Output
C
...
C
C        Retrive OUTPUT_SPK_TYPE  value and check it. Parameter KOSPTP
C        declared in include file.
C
...
C
C        Retrive IGNORE_FIRST_LINES value.Parameter KIGNFL declared 
C        in include file. 
C
...
C
C        Retrive LINES_PER_RECORD value. Parameter KLNPRC declared 
C        in include file.  
C
...
C
C        Retrive EPOCH_STR_LENGTH value. Parameter KELENG declared 
C        in include file. 
C
...
C
C        Retrive POLYNOM_DEGREE value. Parameter KPLDEG declared 
C        in include file.  
C
...
C
     ...
     .                    'setup file keyword ''#'' is not '         //
     .                    'recongnized. Refer to the User''s Guide ' //
     .                    'for the program for the list of '         //
...
C$Procedure   SETUPA ( Get LSK and PCK filenames and load its content )
...
C     filenames but loads corresponding files using the pool or
C     binary loaders. 
C      
C$ Examples
C      
