C$Procedure      SPKR03 ( Read SPK record from segment, type 3 )
...
C$ Abstract
C
C     Read a single SPK data record from a segment of type 3
C     (Chebyshev coefficients, position and velocity).
C
C$ Disclaimer
C
...
C     structure of a data type 3 (Chebyshev polynomials, position
C     and velocity) segment.
C
C     On not so close inspection, you will see that this routine is
C     identical to SPKR02, which reads a type 2 (Chebyshev polynomials,
C     position only) segment.
C
C$ Examples
C
...
C$ Index_Entries
C
C     read record from type_3 spk segment
C
C-&
