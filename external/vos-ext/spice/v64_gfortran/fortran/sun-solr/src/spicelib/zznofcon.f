...
C         loaded CK files. You can use CKBRIEF with the -dump option to
C         display coverage intervals of a CK file.
C
C     2)  At epoch 2.8149297000000E+08 TDB (2008 DEC 02 12:29:30.000
C         TDB), there is insufficient information available to
C         transform from reference frame -82000 (CASSINI_SC_COORD) to
C         reference frame 1 (J2000). CASSINI_SC_COORD is a CK frame; a
C         CK file containing data for instrument or structure -82000 at
C         the epoch shown above, as well as a corresponding SCLK
C         kernel, must be loaded in order to use this frame. Failure to
C         find required CK data could be due to one or more CK files
C         not having been loaded, or to the epoch shown above lying
...
C         data for instrument or structure -41001 at the epoch shown
C         above, as well as a corresponding SCLK kernel, must be loaded
C         in order to use this frame. For a CK frame for which the
C         corresponding SCLK kernel has been loaded, failure to find
C         required CK data could be due to one or more CK files not
C         having been loaded, or to the epoch shown above lying within
C         a coverage gap or beyond the coverage bounds of the loaded CK
C         files. You can use CKBRIEF with the -dump option to display
C         coverage intervals of a CK file.
C
C
C$ Restrictions
C
...
C
     ...
     .      //       'files. You can use CKBRIEF with the -dump option '
