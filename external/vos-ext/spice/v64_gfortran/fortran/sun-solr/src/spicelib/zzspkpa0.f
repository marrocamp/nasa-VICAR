...
C     ==============
C
C        ZZSPKPA0 begins by computing the geometric position T(ET) of
...
C        selected, ZZSPKPA0 computes the position of the target body at
...
C        selected, ZZSPKPA0 computes the position of the target body T
...
C$ Examples
C
C     In the following code fragment, SPKSSB and ZZSPKPA0 are used
...
C           CALL ZZSPKPA0 (  IO,    EPOCH, 'J2000', STVGR2,
C       .                 'LT+S', STIO,   LT              )
C
...
C
C     2) The ephemeris files to be used by ZZSPKPA0 must be loaded
