C$Procedure      SPKBSR ( S/P Kernel, Buffer segments for readers )
...
C     --------  ---  --------------------------------------------------
C     FNAME      I   SPKLEF
C     HANDLE    I/O  SPKLEF, SPKUEF, SPKSFS
C     BODY       I   SPKSFS
C     ET         I   SPKSFS
C     DESCR      O   SPKSFS
C     IDENT      O   SPKSFS 
...
C$ Detailed_Input
C
C     FNAME      is the name of an SPK file to be loaded.
C
C     HANDLE     on input is the handle of an SPK file to be
C                unloaded.
C
...
C$ Detailed_Output
C
C     HANDLE     on output is the handle of the S/P-kernel file
...
C$ Parameters
C
C     FTSIZE     is the maximum number of ephemeris files that can be
C                loaded by SPKLEF at any given time for use by the
C                readers.
C
C     BTSIZE     is the maximum number of bodies whose segments can be
C                buffered by SPKSFS.
C
C     STSIZE     Maximum number of segments that can be buffered at any
C                given time by SPKSFS.
C
C
C$ Exceptions
C
C     1) If SPKBSR is called directly, the error 'SPICE(BOGUSENTRY)'
C        is signaled.
C
C     2) See entry points SPKLEF, SPKUEF, and SPKSFS for exceptions
C        specific to them.
C
C$ Files
C
C     S/P-kernel ephemeris files are indicated by filename before
C     loading (see SPKLEF) and handle after loading (all other places).
C
C$ Particulars
C
C     SPKBSR serves as an umbrella, allowing data to be shared by its
C     entry points:
C
C        SPKLEF       Load ephemeris file.
C        SPKUEF       Unload ephemeris file.
C        SPKSFS       Select file and segment.
C
C     Before a file can be read by the S/P-kernel readers, it must be
C     loaded by SPKLEF, which among other things, loads the file into
C     the DAF system.
C
...
C
C     Once an SPK file has been loaded, it is assigned a file
C     handle, which is used to keep track of the file internally, 
C     and which is used by the calling program to refer to the file 
C     in all subsequent calls to SPK routines.
C
C     A file may be removed from the list of files for potential
C     searching by unloading it via a call to SPKUEF.
C
C     SPKSFS performs the search for segments within a file for the
C     S/P-kernel readers.  It searches through last-loaded files first.
C     Within a single file, it searches through last-inserted segments
C     first, thus assuming that "newest data is best".
C
C     Information on loaded files is used by SPKSFS to manage a buffer
C     of saved segment descriptors and identifiers to speed up access
C     time without having to necessarily perform file reads.
C
C$ Examples
C
C     Suppose that ephemeris data for the Mars Global Surveyor 
C     spacecraft relative to Mars are contained in three separate files:
C     PREDICT.SPK contains complete predict ephemeris data for several
C     successive orbits, and UPDATE_1.SPK and UPDATE_2.SPK contain two
C     separate updates to selected intervals within those orbits, based
C     on altimeter fits.
C
C     In the following example, states of the spacecraft are computed
C     in two different ways:
C
C     First, the predict file and one of the update files are both
C     loaded and states are requested for regular intervals within
C     the orbits. The update file is searched through first, and if no
C     data for the requested time is available, the predict file is
C     used.
C
C     Then, the first update file is unloaded, the second update file
C     is loaded, and the same requests are made as above.
C
C     Throughout the two searches, a table is written which contains
C     the state (position and velocity) of the spacecraft, and the
C     file from which the data came, if such data was found, and an
C     error message otherwise.
C
C     It is assumed that the beginning and ending ephemeris times
C     (BEG_ET, END_ET) for the entire span have already been
C     initialized, along with the step-size for each measurement
C     (DELTA). The two routines WRITE_TABLE and WRITE_ERROR do not
C     exist in SPICELIB.
C
C
C           INTEGER               PRED_HNDL
...
C           INTEGER               CENTER
C
C           DOUBLE PRECISION      BEG_ET
C           DOUBLE PRECISION      END_ET
...
C
C     C
C     C     Load the predict file and the first update file.  Since
C     C     last-loaded files get searched first, we want to load the
C     C     update file second.
C     C
C           CALL SPKLEF ( 'PREDICT.SPK',  PRED_HNDL )
C           CALL SPKLEF ( 'UPDATE_1.SPK', UPD1_HNDL )
C
C     C
C     C     NAIF code for the Mars Global Surveyor spacecraft is -94.
C     C
C           BODY = -94
C
C     C
C     C     Compute states for regular intervals between BEG_ET and
C     C     END_ET.
C     C
C           ET = BEG_ET
C
...
C              IF ( FOUND ) THEN
C     C
C     C           Evaluate the state, get the name of the file from
C     C           whence the data came, and write the results to the
C     C           table.
C     C
C                 CALL SPKPV ( HANDLE, DESCR, ET, 'J2000', STATE,
C             .                CENTER )
C
C                 CALL DAFHFN ( HANDLE, FNAME )
C
C                 CALL WRITE_TABLE ( ET, STATE, FNAME )
C
C              ELSE
C
C                 CALL WRITE_ERROR ( ET )
C
C              END IF
C
C     C
...
C
C     C
C     C     Unload the first update file, load the second, and do
C     C     everything over again.  Since the original file stays
C     C     loaded, the update file once again gets searched first.
C     C
C           CALL SPKUEF (  UPD1_HNDL )
C           CALL SPKLEF ( 'UPDATE_2.SPK', UPD2_HNDL )
C
C           ET = BEG_ET
C
C           DO WHILE ( ET .LE. END_ET )
C
C     C
C     C        Locate the applicable segment.
C     C
C              CALL SPKSFS ( BODY, ET, HANDLE, DESCR, IDENT, FOUND )
C
C              IF ( FOUND ) THEN
...
C$ Restrictions
C
C     1) If Fortran I/O errors occur while searching a loaded SPK
C        file, the internal state of this suite of routines may
C        be corrupted.  It may be possible to correct the state
C        by unloading the pertinent SPK files and then re-loading
C        them.
C
...
C$ Version
C
C-    SPICELIB Version 5.2.0, 07-APR-2010 (NJB)
C
C        Increased segment table buffer size to 50000 entries.
C
C-    SPICELIB Version 5.1.0, 08-SEP-2005 (NJB)
C
C        Updated to remove non-standard use of duplicate arguments
C        in MOVED calls in entry points SPKUEF and SPKSFS.     
C
C        Increased segment table buffer size to 30000 entries.
C
C-    SPICELIB Version 5.0.0, 21-FEB-2003 (NJB)
C
C        Increased segment table buffer size to 10000 entries.
C
...
C              is contributed by a single SPK file, and the list is
...
C              DAFCLS.
C
C           4) The load routine SPKLEF now resets all file numbers when 
C              the next file number reaches INTMAX()-1, thereby 
C              avoiding arithmetic overflow.  
C
C           5) The unload routine SPKUEF now calls RETURN() on entry and
C              returns if so directed.
C
C           6) In SPKSFS, DAF calls are followed by tests of FAILED()
C              in order to ensure that the main state loop terminates.
C
C           7) In SPKSFS, a subscript bound violation in a loop
...
C        boundary problem should be installed in SPICELIB by 
C        October 1995
C
C-    SPICELIB Version 2.0.0, 25-NOV-1992 (JML)
C
C     1) When loading a file, SPKLEF now checks if the file table is
...
C
C     2) A bug in the way that SPKLEF and SPKUEF clean up the body
...
C
C     4) A cut and paste error in the description of the segment
C        table was corrected.
C
...
C$ Index_Entries
C
C     buffer spk segments for readers
C
C-&
...
C
C     Nobody has any business calling SPKBSR directly.
C
      IF ( RETURN() ) THEN
         RETURN
      END IF
...
C$Procedure SPKLEF ( S/P Kernel, Load ephemeris file )
 
      ENTRY SPKLEF ( FNAME, HANDLE )
 
C$ Abstract
C
C     Load an ephemeris file for use by the readers.  Return that
C     file's handle, to be used by other SPK routines to refer to the
C     file.
C
C$ Disclaimer
C
...
C     FTSIZE     P   Maximum number of loaded SPK files.
C    
C$ Detailed_Input
C
...
C                Almost every other SPK routine will subsequently use
C                this number to refer to the file.
C
C$ Parameters
C
C     FTSIZE     is the maximum number of SPK files that may 
...
C     handle by SPKLEF, which will be used by most other routines to
C     refer to it.
C
C$ Particulars
C
C     If there is room for a new file in the file table, SPKLEF creates
C     an entry for it and loads the file for reading using DAFOPR.
C
C$ Examples
C
C     See the Example above, in SPKBSR.
C
C$ Restrictions
C
C     None.
...
C        there are no more segments buffered for a particular body,
C        the counter variable for the bodies is no longer incremented.
C
C        The following code fragment changed:
C
C           IF ( BTBEG( I ) .EQ. 0 ) THEN
C
C              .
C              .
C              .
...
C           END IF
C
C-    Beta Version 1.1.0, 25-JAN-1990 (IMU)
C
C        If a file that has already been loaded is loaded a second
C        (or third or fourth) time, it should be removed from the
C        file table, and any segments from the file must be removed
C        from the segment lists, just as if the user had unloaded
C        the file before loading it again. This means that a single
C        file cannot occur more than once in the file table.
C
C-&
...
C     this is as good a place as any to initialize the segment table 
C     linked list pool, whenever the body table is empty.
C
      IF ( NBT .EQ. 0 ) THEN
...
C        the SPK file.  Remove this link.
C
         CALL DAFCLS ( HANDLE )
...
C 
     ...
     .                    'with # entries.'         )
            CALL ERRINT ( '#', FTSIZE               )
            CALL SIGERR ( 'SPICE(SPKFILETABLEFULL)' )
            CALL CHKOUT ( 'SPKLEF'                  )
            RETURN
...
C$Procedure SPKUEF ( S/P Kernel, Unload ephemeris file )
 
      ENTRY SPKUEF ( HANDLE )
 
C$ Abstract
C
C     Unload an ephemeris file so that it will no longer be searched by
C     the readers.
C
C$ Disclaimer
C
...
C     SPK system, no action is taken.
C
C$ Examples
C
C     See the Example above, in SPKBSR.
...
C-    SPICELIB Version 2.0.0, 25-NOV-1992 (JML)
C
C     1) A bug in the way that SPKLEF and SPKUEF clean up the body
C        tables after a file is unloaded was fixed.
C
...
C        following the header.
C
C-    SPICELIB Version 1.1.0, 2-MAY-1990 (RET)
C
C        If unloading a file causes all segments in the list for a
C        body to go away, delete that body from the body list.
C
C-    SPICELIB Version 1.0.1, 22-MAR-1990 (HAN)
C
C        Literature references added to the header.
...
C        segments buffered for a particular body, the counter variable
C        for the bodies is no longer incremented.
C
C        The following code fragment changed:
C
C           IF ( BTBEG( I ) .EQ. 0 ) THEN
...
C-    Beta Version 1.1.0, 25-JAN-1990 (IMU)
C
C        When unloading a file, close it.
C
C-&
...
C$Procedure SPKSFS ( S/P Kernel, Select file and segment )
...
C$ Exceptions
C
C     1) If an attempt is made to call SPKSFS when there aren't any
C        files loaded, the error SPICE(NOLOADEDFILES) is signaled.
C
C$ Files
C
C     All files loaded by SPKLEF are potential search targets for
C     SPKSFS.
C
C$ Particulars
C     
C     This routine finds the highest-priority segment, in any loaded
C     SPK file, such that the segment provides data for the specified
C     body and epoch.
C
C$ Examples
C
...
C-    SPICELIB Version 1.1.0, 2-MAY-1990 (RET)
C
C        If an attempt is made to call SPKSFS when there are no files
C        loaded, an error is now signaled.
C
C-&
...
C     of each table is provided in the declarations section of SPKBSR. 
 
C
C     There must be at least ONE file loaded.
C
      IF ( NFT .EQ. 0 ) THEN
         CALL SETMSG ( 'At least one SPK file needs to be loaded '   //
     .                 'by SPKLEF before beginning a search.'       )
...
C
C           Now search the loaded SPK files for segments relating to
...
C     at this point.  Also clear the re-use interval's expense.
C
      IF ( BINDEX .GT. 0 ) THEN
