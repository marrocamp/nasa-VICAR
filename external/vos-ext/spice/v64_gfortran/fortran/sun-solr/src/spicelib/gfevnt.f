...
C                      QPNAMS(10) = 'DVEC'
C
C                   Only 'SUB-OBSERVER POINT' searches make 
...
C         SPICE(VALUEOUTOFRANGE) is signaled by routines called by
C         this routine.
C
...
C     call GFSEP, GFDIST, GFPOSC, GFSUBC, GFRR, or GFSNTC rather than
C     this routine. 
C        
...
C$ Index_Entries
C
C     GF low-level geometric condition solver
C
C-&
...
C
C     BDNMLN is the maximum length of a body name.
C
      INTEGER               BDNMLN
      PARAMETER           ( BDNMLN = 36 )
...
C
C     ABCLEN is the maximum length of any aberration correction
C     specification string.  Room for expansion is included.
C
      INTEGER               ABCLEN
      PARAMETER           ( ABCLEN = 20 )
...
C 
         ...
         SRCPRE ( 1, PHASE  ) = 'Phase search pass 1 of #'
         SRCPRE ( 2, PHASE  ) = 'Phase search pass 2 of #'
...
C
         CALL ZZGFSPIN( OF, OBSRVR, SHAPE, FRAME, REFVAL, ABCORR )

         CALL ZZGFREL ( UDSTEP,        UDREFN,   ZZGFSPDC, 
     .                  ZZGFSPLT,      ZZGFGSEP, ZZGFSPUR, 
     .                  OP,            REFVAL,   TOL,  
     .                  ADJUST,        CNFINE,   MW,      NW,
     .                  WORK,          RPT,      UDREPI,  
...
C
         CALL ZZGFDIIN ( TARGET, ABCORR, OBSRVR, REFVAL )

         CALL ZZGFREL ( UDSTEP,         UDREFN,   ZZGFDIDC, 
     .                  ZZGFDILT,       ZZGFDIGQ, ZZGFDIUR,
     .                  OP,             REFVAL,   TOL, 
     .                  ADJUST,         CNFINE,   MW,     NW,
     .                  WORK,           RPT,      UDREPI,
     .                  UDREPU,         UDREPF,   RPTPRE,
...
C
C        Set the interval for the QDERIV call in ZZGFRRDC to one
C        TDB second.
C
         DT = 1.D0

         CALL ZZGFRRIN ( TARGET, ABCORR, OBSRVR, REFVAL, DT )

         CALL ZZGFREL ( UDSTEP,           UDREFN,   ZZGFRRDC,
     .                  ZZGFRRLT,         ZZGFRRGQ, ZZGFRRUR,
     .                  OP,               REFVAL,   TOL,
     .                  ADJUST,           CNFINE,   MW,      NW,
     .                  WORK,             RPT,      UDREPI,
     .                  UDREPU,           UDREPF,   RPTPRE,
