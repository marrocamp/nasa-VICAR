...
C                represented as encoded ticks.  The values contained
C                in PSTART are whole numbers.
C
C     PSTOP      is an array containing NPARTS partition end times
C                represented as encoded ticks. The values contained
C                in PSTOP are whole numbers.
C
C$ Parameters
C
...
C
C     The start and stop times returned are in encoded units of "ticks".
C
C$ Examples
C
