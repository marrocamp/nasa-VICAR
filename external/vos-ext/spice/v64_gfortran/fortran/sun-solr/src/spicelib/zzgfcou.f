C$Procedure   ZZGFCOU ( GF, coordinate utility package )
     ...
     .                     CRDNAM, REFVAL, DECRES, LSSTHN, CRDVAL, 
     .                     CRDFND                                 )
...
C     METHOD     I   COIN
C     TARGET     I   COIN
C     ET         I   COIN, CODC, COLT, COG, COCD, COCL, COCG, COSD,
C                    COSL, COSG, COEX
C     REF        I   COIN
C     ABCORR     I   COIN
C     OBSRVR     I   COIN
...
C
C        ZZGFCOUR      updates the reference value, REFVAL.  REFVAL
C                      serves as the comparison value for equality
C                      or inequality relations.
C
...
C                      time.
C
C        ZZGFCOLT      indicates whether the coordinate is less than
C                      the reference value REFVAL at a specified time.
C
...
C                      specified time.
C
C        ZZGFCOCL      indicates whether the cosine of the coordinate is
C                      less than the reference value REFVAL at a
C                      specified time.
C
C        ZZGFCOSL      indicates whether the sine of the coordinate is
C                      less than the reference value REFVAL at a
C                      specified time.
C
C$ Examples
...
C$Procedure ZZGFCOIN ( GF, coordinate search initialization )
     ...
     .                 DREF,   DVEC,   CRDSYS, CRDNAM, REFVAL         )
...
C                be non-empty.               
C
C                REFVAL is ignored if OP is not an equality or
...
C$Procedure ZZGFCOUR ( GF, update reference value )
 
      ENTRY ZZGFCOUR ( REFVAL )
 
C$ Abstract
C
C     Update the reference value set by the last call to ZZGFCOIN.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C$ Detailed_Input
C
C     REFVAL         is the new reference value to be used by
C                    the entry points 
C
C                       ZZGFCODC
C                       ZZGFCOCD
C                       ZZGFCOSD
C
C                    in this package.  The coordinate, or the cosine or
C                    sine of the coordinate, is compared to the
C                    reference value by these entry points.
C
C$ Detailed_Output
C
C     None.
...
C$ Particulars
C
C     This routine supports use of GFREL within ZZGFLONG.  This
C     routine is used as the actual argument corresponding to 
C     GFREL's dummy argument GFQREF.
C
C$ Examples
C
C     See ZZGFLONG.
...
C$Procedure ZZGFCOLT  ( GF, is coordinate less than reference value? )
 
      ENTRY ZZGFCOLT ( ET, LSSTHN )

C$ Abstract
C
C     Indicate whether the coordinate defined by the last call to
C     ZZGFCOIN is less than the reference value.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C     --------  ---  --------------------------------------------------
C     ET         I   Computation epoch.
C     LSSTHN     O   Flag indicating if "less than" relation holds.
C
C$ Detailed_Input
C
C     ET             is the computation epoch, expressed as seconds
...
C$ Detailed_Output
C
C     LSSTHN         is a logical flag indicating whether the cosine of
C                    the coordinate defined by the previous call to
C                    ZZGFCOIN is strictly less than the reference value
C                    at the epoch ET.  LSSTHN is .TRUE. if this
C                    relation holds and .FALSE. otherwise.
C$ Parameters
C
C     None.
C
...
C$ Index_Entries
C
C     is coordinate less than reference value
C
C-&
     ...
     .               SVRCTR, SVRE,   SVF,    SVCRD,  VALUE,  FOUND )
      ...
      IF ( .NOT. FOUND ) THEN
         ...
         CALL ERRCH  ( '#', TIMSTR                                   )
         CALL SIGERR ( 'SPICE(NOTCOMPUTABLE)'                        )
         CALL CHKOUT ( 'ZZGFCOLT'                                    )
         RETURN

      END IF


      LSSTHN  =  VALUE .LT. SVRVAL
...
C$Procedure ZZGFCOCL ( GF, is cosine of coordinate < reference value? )
 
      ENTRY ZZGFCOCL ( ET, LSSTHN )
 
C$ Abstract
C
C     Indicate whether the cosine of the coordinate defined by the
C     last call to ZZGFCOIN is less than the reference value.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C$ Index_Entries
C
C     is cosine of coordinate less than reference value
C
C-&
...
C
C     Compute the coordinate; compare the cosine to the reference
C     value.
C     
      CALL ZZGFCOQ ( SVVDEF, SVMETH, SVTARG, ET,     SVREF,      
     .               SVCORR, SVOBS,  SVDREF, SVDVEC, SVCSYS,
     .               SVRCTR, SVRE,   SVF,    SVCRD,  VALUE, FOUND )
      ...
      IF ( .NOT. FOUND ) THEN
         ...
         CALL ERRCH  ( '#', TIMSTR                                   )
         CALL SIGERR ( 'SPICE(NOTCOMPUTABLE)'                        )
         CALL CHKOUT ( 'ZZGFCOCL'                                    )
         RETURN

      END IF         

      LSSTHN  =  COS ( VALUE ) .LT. SVRVAL
...
C$Procedure ZZGFCOSL ( GF, is sine of coordinate < reference value? )
 
      ENTRY ZZGFCOSL ( ET, LSSTHN )
...
C$ Detailed_Output
C
C     LSSTHN         is a logical flag indicating whether the sine of
C                    the coordinate defined by the previous call to
C                    ZZGFCOIN is strictly less than the reference value
C                    at the epoch ET.  LSSTHN is .TRUE. if this
C                    relation holds and .FALSE. otherwise.
...
C$ Index_Entries
C
C     is sine of coordinate less than reference value
C
C-&
...
C
     ...
     .               SVRCTR, SVRE,   SVF,    SVCRD,  VALUE, FOUND )
      ...
      IF ( .NOT. FOUND ) THEN
         ...
         CALL ERRCH  ( '#', TIMSTR                                   )
         CALL SIGERR ( 'SPICE(NOTCOMPUTABLE)'                        )
         CALL CHKOUT ( 'ZZGFCOSL'                                    )
         RETURN

      END IF         

      LSSTHN  =  SIN ( VALUE ) .LT. SVRVAL
