C$Procedure GFUDS ( GF, user defined scalar )
      ...
      SUBROUTINE GFUDS ( UDFUNC, UDQDEC, RELATE, REFVAL, 
     .                   ADJUST, STEP,   CNFINE,        
     .                   MW,     NW,     WORK,   RESULT  )
...
C                 quantity calculated by UDFUNC is decreasing.
C
...
C     time derivative of UDFUNC is positive or negative at ET. For
C     cases where UDFUNC is numerically well behaved, the user 
...
C     derivative of UDFUNC using a three-point estimation.
C     Please see the Examples section for an example of GFDECR use.
C
C           SUBROUTINE GFDECR ( UDFUNC, ET, ISDECR )      
C           IMPLICIT NONE
C
C           EXTERNAL              UDFUNC
C           EXTERNAL              UDDF
...
C
C           CALL UDDC ( UDFUNC, ET, DT, ISDECR )
C
C           END
C
...
C     C
C     C     UDFUNC - the user defined scalar quantity function.
C     C
C           CALL UDDC ( UDFUNC, ET, DT, ISDECR )
C     
C           END
...
C
C     Use the default GF convergence tolerance.
C
      TOL = CNVTOL
