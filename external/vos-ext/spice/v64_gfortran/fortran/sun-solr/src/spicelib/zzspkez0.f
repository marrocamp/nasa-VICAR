C$Procedure ZZSPKEZ0 ( S/P Kernel, easy reader )
 
      SUBROUTINE ZZSPKEZ0 ( TARG, ET, REF, ABCORR, OBS, STARG, LT )
...
C
C        Removed "C$" markers from text blocks in the header.
C
...
C
      IF ( USEGEO ) THEN
         ...
         CALL ZZSPKGO0 ( TARG, ET, REF, OBS, STARG, LT )
...
C
         IF ( TYPE .EQ. INERTL ) THEN
            ...
            CALL ZZSPKAC0 ( TARG, ET, REF, ABCORR, OBS, STARG, LT, DLT )
            CALL CHKOUT   ( RNAME )
            RETURN
...
C
         CALL ZZSPKAC0 ( TARG, ET,    'J2000', ABCORR, 
     .                   OBS,  STATE, LT,      DLT     )
         ...
         ELSE
            ...
            CALL ZZSPKSB0 ( OBS,    ET,     'J2000',         STOBS )
            CALL ZZSPKLT0 ( CENTER, ET,     'J2000', ABCORR, STOBS,
     .                      TEMP,   LTCENT, DLTCTR                 )
         END IF
...
C
         CALL ZZFRMCH0 ( FJ2000, REQFRM, ET + LTSIGN*LTCENT, XFORM )
