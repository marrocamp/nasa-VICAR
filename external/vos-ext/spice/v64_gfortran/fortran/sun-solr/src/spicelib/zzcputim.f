...
C
      ...
      END IF
C
C     Call the Fortran library routines to get the date and
C     time. DATE contains the day, month, year. TIME contains the
C     hour, minute, second.
C
      CALL IDATE ( DMY )
      CALL ITIME ( HMS )
C
C     Now get the date a second time. We do this just in case between
C     the first call and the second call we passed through midnight.
C     If we did pass through midnight, we need the new hour, minute, and
C     second, and we need to assign the new date to DATE.
C
      CALL IDATE ( DMY2 )
 
      IF ( DMY(1) .NE. DMY2(1) ) THEN
      ...
      END IF
C
C     Now, convert the date and time to double precision values, and
C     load them into the output vector in the correct order: year,
C     month, day, hour, minute, second.
C
      TVEC(1) = DBLE( DMY(3) )
      TVEC(2) = DBLE( DMY(2) )
      TVEC(3) = DBLE( DMY(1) )
