...
C        ==============
C
C        SPKEZR begins by computing the geometric position T(ET) of the
...
C        for ABCORR, SPKEZR computes the position of the target body at
...
C        selected, SPKEZR computes the position of the target body T at
...
C                  ET = ET0 + (I-1)*STEP
C
C                  CALL SPKEZR ( TARGET, ET, FRAME, ABCORR, OBSRVR, 
C              .                 STATE,  LT                        )
C
C                  WRITE (*,*) 'ET = ', ET
...
C
      CALL ZZBODN2C ( TARG, TARGID, FOUND )
      ...
      IF ( .NOT. FOUND ) THEN
         ...
         IF ( BEINT(TARG) ) THEN
            ...
            CALL NPARSI ( TARG, TARGID, ERROR, PTR )
...
C
      ...
      IF ( .NOT. FOUND ) THEN
         ...
         IF ( BEINT(OBS) ) THEN
            ...
            CALL NPARSI ( OBS, OBSID, ERROR, PTR )
