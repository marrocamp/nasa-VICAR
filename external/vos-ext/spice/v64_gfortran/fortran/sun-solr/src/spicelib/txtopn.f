C$Procedure      TXTOPN ( Text file, open new )
 
      SUBROUTINE TXTOPN ( FNAME, UNIT )
 
C$ Abstract
C
C     Open a new text file for subsequent write access.
C
C$ Disclaimer
C
...
C
C     TXTOPN opens a new text file and makes use of the SPICELIB
...
C
C-    SPICELIB Version 2.0.0, 07-APR-1998 (NJB)
C
C        Module was updated for the PC-LINUX platform.
C
C-    SPICELIB Version 1.2.0, 11-NOV-1993 (HAN)
C
...
C
C        The code was reformatted so that a utility program can
