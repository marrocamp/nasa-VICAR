C$Procedure SPKW02 ( Write SPK segment, type 2 )
...
C$ Detailed_Input
C
C     HANDLE         DAF handle of an SPK file to which a type 2 segment
...
C                    record).  Each set of Chebyshev coefficients must
C                    cover this fixed time interval, INTLEN.
C
C     N              Number of sets of Chebyshev polynomial coefficients
C                    for coordinates (number of logical records) to be
C                    stored in the segment.  There is one set of
C                    Chebyshev coefficients for each time period.
C
C     POLYDG         Degree of each set of Chebyshev polynomials, i.e.
C                    the number of Chebyshev coefficients per coordinate
C                    minus one.
C
...
C                    logical record).  FIRST is an appropriate value
C                    for BTIME.
C
C$ Detailed_Output
C
...
C     C
C            SEGID = 'MY_SAMPLE_SPK_TYPE_2_SEGMENT'
C
C     C
C     C      Write the segment.
C
C            CALL SPKW02 ( HANDLE, 301,    3,      'J2000',
...
C
C-    SPICELIB Version 1.0.0, 1-AUG-1995 (KSZ)
C
C-&
