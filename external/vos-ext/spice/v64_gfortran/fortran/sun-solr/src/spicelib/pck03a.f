C$Procedure      PCK03A ( PCK type 03: Add data to a segment )
 
      SUBROUTINE PCK03A ( HANDLE, NCSETS, COEFFS, EPOCHS )
 
C$ Abstract
C
C     Add data to a type 03 PCK segment in the binary PCK file
C     associated with HANDLE. See also PCK03B and PCK03E.
C
C$ Disclaimer
C
...
C     NCSETS     I   The number of Cheby coefficient sets and epochs.
C     COEFFS     I   The collection of Cheby coefficient sets.
...
C
C     NCSETS   is the number of Cheby coefficient sets and epochs
C              to be stored in the segment.
C
C     COEFFS   contains a time-ordered array of Chebyshev coefficient
C              sets for computing the orientation of a body relative to
C              the an inertial frame. The orientation is defined by
C              the angles RA, DEC, W and body fixed angular rates for
C              each axis of the body fixed coordinate system defined by
C              RA, DEC, and W. All of the angles and the angular rates
...
C              corresponding to the elements in COEFFS.  The I'th
C              epoch must equal the epoch of the I'th set of
C              coefficients. The epochs must form a strictly increasing
C              sequence.
C
C$ Detailed_Output
C
C     None.    The data is stored in a segment in the binary PCK file
...
C$ Particulars
C
C     This routine adds data to a type 03 PCK segment in the binary
C     PCK file that is associated with HANDLE. The segment must have
C     been begun by calling PCK03B.
C
...
C$ Restrictions
C
C     1) The type 03 PCK segment to which we are adding data must have
C        been started by the routine PCK03B, the routine which begins a
C        type 03 PCK segment.
C
C$ Exceptions
C
...
C$ Index_Entries
C
C     add data to a type_03 pck segment
C
C-&
...
C
     ...
     .   //            ' to be added to the PCK segment in the file'
