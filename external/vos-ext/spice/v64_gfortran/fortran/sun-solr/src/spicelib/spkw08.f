...
C        error SPICE(INVALIDDEGREE) is signalled.
C
C     5) If the number of states N is not at least DEGREE+1, the error
C        SPICE(TOOFEWSTATES) will be signalled.
C
...
C        be signalled.
C
C     8) If the first epoch EPOCH1 is greater than FIRST, the error
C        SPICE(BADDESCRTIMES) will be signalled.
C
C     9) If the last epoch
C
C           FIRST + (N-1)*STEP
C
