...
C$Procedure      PCKOPN ( PCK, open new file. )
 
      SUBROUTINE PCKOPN ( NAME, IFNAME, NCOMCH, HANDLE )
 
C$ Abstract
C
C     Create a new PCK file, returning the handle of the opened file.
C
C$ Disclaimer
C
...
C     --------  ---  --------------------------------------------------
C     NAME       I   The name of the PCK file to be opened.
C     IFNAME     I   The internal filename for the PCK.
C     NCOMCH     I   The number of characters to reserve for comments.
C     HANDLE     O   The handle of the opened PCK file.
C
C$ Detailed_Input
C
C     NAME     The name of the PCK file to be created.
C
C     IFNAME   The internal filename for the PCK file that is being
C              created. The internal filename may be up to 60 characters
C              long. If you do not have any conventions for tagging your
C              files, an internal filename of 'PCK_file' is perfectly
C              acceptable. You may also leave it blank if you like.
C
C     NCOMCH   This is the space, measured in characters, to be
C              initially set aside for the comment area when a new PCK
C              file is opened. The amount of space actually set aside
C              may be greater than the amount requested, due to the
C              manner in which comment records are allocated in an PCK
...
C$ Detailed_Output
C
C     HANDLE   The handle of the opened PCK file. If an error occurs
...
C$ Particulars
C
C     Open a new PCK file, reserving room for comments if requested.
C
C$ Examples
C
