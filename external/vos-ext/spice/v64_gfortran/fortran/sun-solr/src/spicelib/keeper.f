...
C
C     9) The error 'SPICE(BADVARNAME)' signals from a routine in the
...
C
      ...
      CALL STPOOL( 'KERNELS_TO_LOAD', FILNUM, '+', FIL2LD, FNMLEN, OK )
...
C
         ...
         CALL STPOOL( 'KERNELS_TO_LOAD', FILNUM,  '+',
     .                 FIL2LD,           FNMLEN,  OK   )
...
C$ Abstract
C
C     Clear the KEEPER system:  unload all kernels, clear the kernel
C     pool, and re-initialize the system.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C     inserted via the P*POOL API routines.
C
C$ Literature_References
C
C     None.
...
C      
C     Unloading Text or Meta-text Kernels.
C
C     Part of the action of unloading text (or meta-text kernels) is
