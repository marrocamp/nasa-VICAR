...
C      LLMAX     P    Maximum length of line.
C
C$ Detailed_Input
C
...
C
C     LLMAX      is the maximum length of the line. This value should
...
C$ Particulars
C
C     This routine will try to form the best looking assingment, taking
...
C
C        Put the DO WHILE constuct back in and replace WRITE
...
C     have to let it extend past LL, but not past LLMAX.
C
      VLMAX = LEN(V)
      KL    = RTRIM(K)
...
C
C     Same thing over again, exept we have no keyword, or equal sign.
C
