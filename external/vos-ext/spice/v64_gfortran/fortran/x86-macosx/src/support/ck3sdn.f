...
C     The elimination, refered to in these comments as "down sampling",
...
C
C           Check all records between the currently picked window ends
C           to see if interpolated pointing is within tolerance of the
C           actual pointing.
C
            FITOK = .TRUE.
...
C
               ANGLE  =  4.D0  * ASIN ( SQRT(DIST2) / 2.D0 )
...
C
C              If right side when "over" the left side, set left side
