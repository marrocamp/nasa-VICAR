...
C     set of coordinate pairs, at a specified abcissisa value.
C
C$ Disclaimer
C
...
C                    scratch area to hold intermediate results.  WORK
C                    is permitted to coincide with YVALS.
C
C
...
C         SPICE(DIVIDEBYZERO) will be signalled.  The function will
C         return the value 0.D0.
C
C     2)  If N is less than 1, the error SPICE(INVALIDSIZE) is
C         signalled.  The function will return the value 0.D0.
C
