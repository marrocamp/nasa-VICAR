...
C     size minus one."  This quantity plays the same role as does the
C     polynomial degree for the type 9 subsetter. Also get the number
C     of records in the segment.
C
...
C     least WNSZM1+1 states in the segment.
C
      REC(2)  =   MAX (  REC(2),  WNSZM1 + 1 )
...
C
C     Store subtype, the window size minus one and the number of
C     records to end the segment.
C
      CALL DAFADA ( DBLE ( SUBTYP ),                  1 )
      CALL DAFADA ( DBLE ( WNSZM1 ),                  1 )
