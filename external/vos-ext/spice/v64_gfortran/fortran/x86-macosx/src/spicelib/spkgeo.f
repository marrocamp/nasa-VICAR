C$Procedure SPKGEO ( S/P Kernel, geometric state )
...
C$ Detailed_Output
C
C     STATE       contains the position and velocity of the target
C                 body, relative to the observing body, corrected
C                 for the specified aberrations, at epoch ET.  STATE
C                 has six elements:  the first three contain the
C                 target's position; the last three contain the target's
...
C$ Particulars
C
C     SPKGEO computes the geometric state, T(t), of the target
...
C     C
C            CALL SPKLEF ( 'SAMPLE.BSP', HANDLE )
C
C            .
C            .
C            .
...
C               ET = BEGIN + I*DELTA
C
C               CALL SPKGEO ( MOON, ET, 'J2000', EARTH, STATE, LT )
C
...
C$ Restrictions
C
C     1) The ephemeris files to be used by SPKGEO must be loaded
C        by SPKLEF before SPKGEO is called.
C
...
C$ Version
C
C-    SPICELIB Version 2.4.0, 01-SEP-2005 (NJB)
C
C        Updated to remove non-standard use of duplicate arguments
C        in VADDG calls.
C
C-    SPICELIB Version 2.3.0, 05-JAN-2005 (NJB)
C
C        Tests of routine FAILED() were added.
C
C-    SPICELIB Version 2.2.1, 20-OCT-2003 (EDW)
C
C        Added mention that LT returns in seconds.
C
C-    SPICELIB Version 2.2.0, 11-APR-1997 (WLT)
C
C        The routine was modified to take advantage of the fact
C        that most state transformation are between inertial frames.
C        Looking up a transformation between inertial frames is
C        substantially faster than looking up non-inertial
C        transformations.  Consequently slightly more
C        complex code produces about a 50% increase in speed for
C        many users.
C
C-    SPICELIB Version 2.1.0, 26-JUL-1996 (WLT)
C
C        The routine was upgraded so that potentially redundant
C        computations are not performed.
C
C-    SPICELIB Version 2.0.0, 19-SEP-1995 (WLT)
C
C        The routine was upgraded so that it can return states
C        relative to rotating frames.
C
...
C
C-    SPICELIB Version 1.0.0, 18-JUL-1991 (JEM)
C
C-&
...
C
C
C     Since Inertial frames are the most extensively used frames
C     we use the more restrictive routine IRFNUM to attempt to
C     look up the id-code for REF.  If IRFNUM comes up empty handed
C     we then call the more general routine NAMFRM.
C
      CALL IRFNUM ( REF, REFID )
 
      IF ( REFID .EQ. 0 ) THEN
         CALL NAMFRM ( REF, REFID )
      END IF
...
C
               ...
               ELSE
                  ...
                  CALL FRMCHG( TFRAME(CHLEN), TMPFRM,   ET,   STXFRM )
...
C
            ...
            ELSE
               ...
               CALL FRMCHG ( CFRAME, TMPFRM, ET,   STXFRM )
...
C
      ...
      DO I = 2, CTPOS-1
         ...
         ELSE
            ...
            CALL FRMCHG ( TFRAME(I),  TFRAME(I+1), ET,  STXFRM       )
...
C
         ...
         ELSE
            ...
            CALL FRMCHG ( CFRAME, REFID, ET,  STXFRM )
...
C        FRMCHG to get things into a common frame.
C
      ...
      ELSE
C
C        Use the more general routine FRMCHG to make the transformation.
C
         CALL FRMCHG ( TFRAME(CTPOS),  CFRAME,          ET,  STXFRM )
