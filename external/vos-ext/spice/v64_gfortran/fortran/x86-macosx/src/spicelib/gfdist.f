...
C$ Abstract
C
C     Return the time window over which a specified constraint on
...
C     distance function is monotone increasing and monotone decreasing.
C     Each of these time periods is represented by a SPICE window.
C     Having found these windows, all of the distance function's local
...
C     most one solution of any equality constraint. With these solutions
C     in hand, solutions of inequalities are easily found as well.
C      
C
C     Step Size
C     =========
...
C     tolerance doesn't limit the accuracy of solutions found by this
...
C     as GFENVT  must be called. Making the tolerance tighter than
...
C     greater affect on processing time than would the convergence
C     tolerance.
C
C
