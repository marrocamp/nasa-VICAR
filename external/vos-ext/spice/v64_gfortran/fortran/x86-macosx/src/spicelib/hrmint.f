...
C     consists of the N-1 values obtained by evaluating the first-degree
C     interpolants at X.  We'll also evaluate the derivatives of these
C     interpolants at X and save the results in the second column of
C     WORK.  Because the derivative computations depend on the 
C     function computations from the previous column in the 
C     interpolation table, and because the function interpolation
C     overwrites the previous column of interpolated function values,
C     we must evalute the derivatives first.
C
...
C        each input abscissa value, evaluated at X.
C  
