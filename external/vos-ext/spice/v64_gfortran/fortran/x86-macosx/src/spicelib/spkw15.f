...
C
C     TP          is a unit vector parallel to the angular momentum
C                 vector of the orbit at epoch expressed relative to
C                 FRAME.
C
C     PA          is a unit vector parallel to the position vector
C                 of the trajectory at periapsis of EPOCH expressed
C                 relative to FRAME.
C
...
C
C     PV           is a unit vector parallel to the north pole vector
C                  of the central body expressed relative to FRAME.
C
...
C        have zero length, the error 'SPICE(BADVECTOR)' is signalled.
C
C     4) If the trajectory pole vector and the periapsis vector are
C        not orthogonal, the error 'SPICE(BADINITSTATE)' is signalled.
...
C     for modelling the motion of your object.)
C
C     C
...
C
      CALL VEQU ( TP, RECORD(2) )
 
C
C     The periapsis vector.
C
      CALL VEQU ( PA, RECORD(5) )
...
C
     ...
     .   //            'cause of this problem is an unititialized '
     .   //            'vector.'            )
     ...
     .   //            'of this problem is an unitialized vector.' )
         CALL SIGERR ( 'SPICE(BADVECTOR)'   )
         CALL CHKOUT ( 'SPKW15'             )
         RETURN
     ...
     .   //            'SPKW15 had length zero. The most likely '
     .   //            'cause of this problem is an unitialized '
     .   //            'vector. '           )
         CALL SIGERR ( 'SPICE(BADVECTOR)'   )
         CALL CHKOUT ( 'SPKW15'             )
         RETURN
