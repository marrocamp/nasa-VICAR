...
C     present to aviod real or potential problems with pedantic
...
C        convert the orginal statement into standard Fortran.
C
C-&
...
C           initial non-blank substring of MODULE.
C
            FIRST  =  FRSTNB ( MODULE )
...
C                           signalled, the traceback at that point is
...
C
C         to obtain the maximum depth reached, MAXDEP woudl have a
...
C     signalled, a copy of the traceback is saved for later retrieval
...
C     trace will usually be invalid at the time an error is signalled.
