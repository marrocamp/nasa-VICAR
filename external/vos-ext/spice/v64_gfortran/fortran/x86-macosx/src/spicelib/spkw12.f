C$Procedure      SPKW12 ( Write SPK segment, type 12 )
 
      SUBROUTINE SPKW12 (  HANDLE,  BODY,    CENTER,  FRAME,
     .                     FIRST,   LAST,    SEGID,   DEGREE,
     .                     N,       STATES,  EPOCH1,  STEP     )
 
C$ Abstract
C
C     Write a type 12 segment to an SPK file.
C
C$ Disclaimer
C
...
C         signaled.
C
C     6)  If the number of states N is not at least (DEGREE+1)/2, the 
C         error SPICE(TOOFEWSTATES) will be signaled.
C
C     7)  If FIRST is greater than LAST then the error
C         SPICE(BADDESCRTIMES) will be signaled.
C
C     8)  If STEP is non-positive, the error SPICE(INVALIDSTEPSIZE) will
C         be signaled.
C
C     9)  If the first epoch EPOCH1 is greater than FIRST, the error
C         SPICE(BADDESCRTIMES) will be signaled.
C
C     10) If the last epoch
C
C           FIRST + (N-1)*STEP
C
C         is less than LAST, the error SPICE(BADDESCRTIMES) will be
C         signaled.
C
C$ Files
C
C     A new type 12 SPK segment is written to the SPK file attached
C     to HANDLE.
C
C$ Particulars
C
C     This routine writes an SPK type 12 data segment to the open SPK
C     file according to the format described in the type 12 section of
...
C     a segment of type 12 in an SPK file.
C
...
C            C
C                  SEGID = 'MY_SAMPLE_SPK_TYPE_12_SEGMENT'
C
C            C
C            C     Write the segment.
C            C
C                  CALL SPKW12 (  HANDLE,  BODY,    CENTER,  FRAME,
...
C$ Index_Entries
C
C     write spk type_12 ephemeris data segment
C
C-&
...
C
      ...
      ELSE IF (   ( EPOCH1 + (N-1)*STEP )  .LT.   LAST  ) THEN
 
         CALL SETMSG ( 'Segment end time # follows last epoch #.'     )
         CALL ERRDP  ( '#',  LAST                                     )
         CALL ERRDP  ( '#',  EPOCH1 + (N-1)*STEP                      )
         CALL SIGERR ( 'SPICE(BADDESCRTIMES)'                         )
         CALL CHKOUT ( 'SPKW12'                                       )
         RETURN
...
C
C     The type 12 segment structure is eloquently described by this
