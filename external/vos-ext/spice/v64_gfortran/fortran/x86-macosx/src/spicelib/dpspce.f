...
C$ Index_Entries
C
C     NORAD two line elements, deep space, Cheyenne Mountain
C
C-&
...
C
C        Initialize the periodic perturbations to the epoch.
C        This is required because perturbations are zero at the
C        epoch.
C
         OMGADF = ELEMS( 7  )
         XNODDF = ELEMS( 5  )
         ...
         CALL ZZDPPER( ZERO, E, XINC, OMGADF, XNODDF, XMAM )
 
 
C
C        Initialization complete.  Rejoice!
C
