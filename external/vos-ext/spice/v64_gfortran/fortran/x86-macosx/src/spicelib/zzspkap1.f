C$Procedure ZZSPKAP1 ( S/P Kernel, apparent state )
 
      SUBROUTINE ZZSPKAP1 ( TARG, ET, REF, SOBS, ABCORR, STARG, LT )
...
C
C
C        3) Obtain an uncorrected state vector derived directly from 
C           data in an SPK file:
C
C              Use 'NONE'.
C
C
C        4) Compute the apparent position of a target body relative
...
C     ==============
C
C        ZZSPKAP1 begins by computing the geometric position T(ET) of
...
C        selected, ZZSPKAP1 computes the position of the target body at
...
C        selected, ZZSPKAP1 computes the position of the target body T
...
C$ Examples
C
C     In the following code fragment, ZZSPKSB1 and ZZSPKAP1 are used
...
C
C           CALL ZZSPKSB1 (  VGR2,   EPOCH,  'J2000',  STVGR2  )
C           CALL ZZSPKAP1 (  IO,     EPOCH,  'J2000',  STVGR2,
C       .                 'LT+S',  STIO,    LT               )
C
...
C
C     2) The kernel files to be used by ZZSPKAP1 must be loaded
...
C
C     4) In a future version of this routine, the implementation 
...
C
C        Based on SPICELIB Version 3.0.1, 20-OCT-2003 (EDW)
C
C-&
...
C     apparent state from spk file
C     get apparent state
C
C-&
...
C
C        In the example program, the calling sequence of ZZSPKAP1
C        was corrected.
C
C-    SPICELIB Version 1.1.0, 25-MAY-1990 (HAN)
C
C        The local variable CORR was added to eliminate a run-time
C        error that occurred when ZZSPKAP1 was determining what
C        corrections to apply to the state. If the literal string
C        'LT' was assigned to ABCORR, ZZSPKAP1 attempted to look at
...
C        Also, without the local variable CORR, ZZSPKAP1 would have to
...
C 
         CALL CMPRSS ( ' ',   0,    ABCORR, CORR2 )
         CALL UCASE  ( CORR2, CORR                )
...
C     light time.
C
      CALL ZZSPKSB1 ( TARG,  ET,   REF, STARG  )
...
C 
      ...
      DO I = 1, MAXITR
         ...
         CALL ZZSPKSB1 ( TARG,  ET + LTSIGN*LT,  REF,  STARG  )
         CALL VSUBG  ( STARG, SOBS,            6,    TSTATE )
         CALL MOVED  ( TSTATE,                 6,    STARG  )
         LT = VNORM  ( STARG ) / CLIGHT()
