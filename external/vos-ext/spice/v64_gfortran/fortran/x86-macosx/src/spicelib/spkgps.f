C$Procedure SPKGPS ( S/P Kernel, geometric position )
...
C                 the SPICELIB subroutine REFCHG.
C
...
C$ Particulars
C
C     SPKGPS computes the geometric position, T(t), of the target
...
C     relative to a common center.  SPKGPS reads as many segments
C     as necessary, from as many files as necessary, using files
C     that have been loaded by previous calls to SPKLEF (load
C     ephemeris file).
C
C     SPKGPS is similar to SPKGEO but returns geometric positions
C     only.
C
C$ Examples
C
...
C               ET = BEGIN + I*DELTA
C
C               CALL SPKGPS ( MOON, ET, 'J2000', EARTH, POS, LT )
C
...
C$ Restrictions
C
C     1) The ephemeris files to be used by SPKGPS must be loaded
C        by SPKLEF before SPKGPS is called.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 05-NOV-2005 (NJB)
C
...
C
               ...
               ELSE
                  ...
                  CALL REFCHG( TFRAME(CHLEN), TMPFRM,   ET,   PSXFRM )
...
C
            ...
            ELSE
               ...
               CALL REFCHG ( CFRAME, TMPFRM, ET,   PSXFRM )
...
C
      ...
      DO I = 2, CTPOS-1
         ...
         ELSE
            ...
            CALL REFCHG ( TFRAME(I),  TFRAME(I+1), ET,  PSXFRM       )
...
C
         ...
         ELSE
            ...
            CALL REFCHG ( CFRAME, REFID, ET,  PSXFRM )
...
C        REFCHG to get things into a common frame.
C
      ...
      ELSE
C
C        Use the more general routine REFCHG to make the transformation.
C
         CALL REFCHG ( TFRAME(CTPOS),  CFRAME,          ET,  PSXFRM )
