C$Procedure ZZGFSOLVX ( Private --- GF, event finding routine )

      SUBROUTINE ZZGFSOLVX ( UDFUNC, UDCOND, UDSTEP,  UDREFN,  BAIL,
...
C
C     Sister routine to ZZGFSOLV. Copy any edits to ZZGFSOLV or
...
C                           at ET.
C
C     UDCOND     the routine that determines if UDFUNC
...
C
C        Modified version of ZZGFSOLV.
C
...
C     a state.
C
      CALL UDCOND ( UDFUNC, CURTIM,  CURSTE )
