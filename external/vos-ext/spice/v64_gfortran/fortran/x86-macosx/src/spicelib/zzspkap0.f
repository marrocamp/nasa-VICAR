...
C     ==============
C
C        ZZSPKAP0 begins by computing the geometric position T(ET) of
...
C        selected, ZZSPKAP0 computes the position of the target body at
...
C        selected, ZZSPKAP0 computes the position of the target body T
...
C$ Examples
C
C     In the following code fragment, ZZSPKSB0 and ZZSPKAP0 are used
...
C
C           CALL ZZSPKSB0 (  VGR2,   EPOCH,  'J2000',  STVGR2  )
C           CALL ZZSPKAP0 (  IO,     EPOCH,  'J2000',  STVGR2,
C       .                 'LT+S',  STIO,    LT               )
C
...
C
C     2) The kernel files to be used by ZZSPKAP0 must be loaded
...
C
C        In the example program, the calling sequence of ZZSPKAP0
C        was corrected.
C
C-    SPICELIB Version 1.1.0, 25-MAY-1990 (HAN)
C
C        The local variable CORR was added to eliminate a run-time
C        error that occurred when ZZSPKAP0 was determining what
C        corrections to apply to the state. If the literal string
C        'LT' was assigned to ABCORR, ZZSPKAP0 attempted to look at
...
C        Also, without the local variable CORR, ZZSPKAP0 would have to
