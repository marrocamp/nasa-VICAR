...
C
      ...
      INTEGER               ANGS2
      PARAMETER           ( ANGS2  = MODEL2 + NELEMS )
 
      INTEGER               MEANMO
      PARAMETER           ( MEANMO = NGEOCN + KNO )
 
C
C     The parameters below give the location of the nutation terms
C     and their rates for the first packet.
C
      INTEGER               NUOB1
      PARAMETER           ( NUOB1 = ANGS1 )
 
      INTEGER               NULN1
      PARAMETER           ( NULN1 = NUOB1   + 1 )
 
      INTEGER               DNUOB1
      PARAMETER           ( DNUOB1 = NULN1  + 1 )
 
      INTEGER               DNULN1
      PARAMETER           ( DNULN1 = DNUOB1 + 1 )
 
 
C
C     The parameters below give the location of the nutation terms
C     and their rates for the second packet.
C
      INTEGER               NUOB2
      PARAMETER           ( NUOB2 = ANGS2 )
 
      INTEGER               NULN2
      PARAMETER           ( NULN2 = NUOB2 + 1 )
 
      INTEGER               DNUOB2
      PARAMETER           ( DNUOB2 = NULN2   + 1 )
 
      INTEGER               DNULN2
      PARAMETER           ( DNULN2 = DNUOB2 + 1 )
...
C
      ...
      DOUBLE PRECISION      N0
      DOUBLE PRECISION      NULON
      DOUBLE PRECISION      NULON1
      DOUBLE PRECISION      NULON2
      DOUBLE PRECISION      NUMER
      DOUBLE PRECISION      NUOBL
      DOUBLE PRECISION      NUOBL1
      DOUBLE PRECISION      NUOBL2
...
C
      IF ( T1 .NE. T2 ) THEN
         ...
         END IF
C
C        Using the different element packets compute nutations in
C        obliquity and longitude.  Note to avoid run-away nutation
C        interpolation we bracket DT1 and DT2 to be between -5 and 5
C        days.
C
         DT1    = BRCKTD ( ET - T1, -FIVDAY, FIVDAY )
         NUOBL1 = RECORD(NUOB1) + DT1*RECORD(DNUOB1)
         NULON1 = RECORD(NULN1) + DT1*RECORD(DNULN1)
 
         DT2    = BRCKTD ( ET - T2, -FIVDAY, FIVDAY )
         NUOBL2 = RECORD(NUOB2) + DT2*RECORD(DNUOB2)
         NULON2 = RECORD(NULN2) + DT2*RECORD(DNULN2)
C
...
C
C        Use the weighting function to compute the nutation in obliquity
C        longitude.
C
         NUOBL = W*NUOBL1 + (1.0D0-W)*NUOBL2
         NULON = W*NULON1 + (1.0D0-W)*NULON2
 
C
C        Compute the mean obliquity at the epoch ET.
C
         CALL ZZMOBLIQ ( ET, MOB, DMOB )
 
C
C        Construct the transformation from mean of date to true of date.
C
         CALL EUL2M (  -MOB-NUOBL, -NULON, MOB, 1, 3, 1, M )
 
C
C        Use the transpose of the matrix just computed to convert
C        S1 and S2 from true of date to mean of date.
C
         CALL MTXV  ( M,       S1(1),  TMPSTA(1) )
         CALL MTXV  ( M,       S1(4),  TMPSTA(4) )
         CALL MOVED ( TMPSTA,  6,      S1        )
         
         CALL MTXV  ( M,       S2(1),  TMPSTA(1) )
         CALL MTXV  ( M,       S2(4),  TMPSTA(4) )
         CALL MOVED ( TMPSTA,  6,      S2        )
  
C
C        Now compute the weighted average of the two true of date
C        states.
C
      ...
      ELSE
         ...
         END IF
C
C        Interpolate the nutation in longitude and obliquity.  Note
C        that to avoid run-away linear interpolation of the nutation
C        angles, we bracket DT1 to be between -5 and 5 days.
C
         DT1   = BRCKTD ( ET - T1, -FIVDAY, FIVDAY )
         NUOBL = RECORD(NUOB1) + DT1*RECORD(DNUOB1)
         NULON = RECORD(NULN1) + DT1*RECORD(DNULN1)
 
C
C        Get the current obliquity.
C
         CALL ZZMOBLIQ ( ET, MOB, DMOB )
 
         CALL EUL2M (  -MOB-NUOBL, -NULON, MOB, 1, 3, 1, M )
 
         CALL MTXV  ( M,       STATE(1),  TMPSTA(1) )
         CALL MTXV  ( M,       STATE(4),  TMPSTA(4) )
         CALL MOVED ( TMPSTA,  6,         STATE     )
 
      END IF
C
C     Finally, convert the mean of date state to J2000.  First get
C     the rotation from J2000 to mean of date.
C
      CALL ZZEPRCSS ( ET, PRECM )
C
C     Now convert STATE to J2000.
C
      CALL MTXV  ( PRECM,   STATE(1),  TMPSTA(1) )
      CALL MTXV  ( PRECM,   STATE(4),  TMPSTA(4) )
      CALL MOVED ( TMPSTA,  6,         STATE     )
