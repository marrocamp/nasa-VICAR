C$Procedure      SPKE02 ( Evaluate Chebyshev polynomials, type 2 )
C
      SUBROUTINE SPKE02 ( ET, RECORD, XYZDOT )
C
C$ Abstract
C
C     Evaluate a single data record from an PCK or SPK segment of type 2
C     (Chebyshev Polynomials, 3 components).
C
C$ Disclaimer
C
...
C     XYZDOT     O   3 components and their derivatives.
C
C$ Detailed_Input
C
...
C                 will give the 3 component and their derivatives.
C
C$ Detailed_Output
C
C     XYZDOT      is a 6-vector. In order, X, Y, Z, X', Y', and Z'.
C                 Units for state evaluations will be km and km/sec.
C                 Units for angles will be radians and radians/sec.
C
C$ Parameters
C
C     None.
...
C     one set each for components X, Y, and Z.  SPKE02
C     calls the routine CHBINT for each set to evalute the polynomial
C     AND its first derivative (which it computes internally) at the
C     input epoch, thereby arriving at the complete state.
C
C$ Examples
C
