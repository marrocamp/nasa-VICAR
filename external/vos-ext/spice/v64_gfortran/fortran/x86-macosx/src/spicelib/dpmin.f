C$Procedure                     DPMIN ( Smallest DP number )
 
      DOUBLE PRECISION FUNCTION DPMIN ( )
...
C
C     Environment: IBM PC, MS (Microsoft) FORTRAN 5.0
C     Value:       -1.79769313486231D+308
C
C     Environment: Macintosh, Language Systems FORTRAN 1.2.1
C     Value:       -1.7D+308
C
...
C      -1.797693134862316D+308 as the minimum negative double precision
C      number, it produced a compilation error. As a result, we changed
C      the number to -1.79769313486231D+308, which did not produce a
C      compilation error.
C
...
C$ Examples
C
C     The following code fragment illustrates the use of DPMIN.
C     Note example that the smallest negative number is not
C     necessarily the negative of the largest positive number.
C
C        C
...
C         FORTRAN, page 306 (LIBM_DOUBLE). This routine includes the
C         function D_MAX_NORMAL.
C
...
C
C         The master program was updated to SAVE the minimum
...
C
C         Module was updated to include the value of DPMIN for the
...
C
C         Module was updated to include the value of DPMIN
C         for the Lahey F77L EM/32 environment (PC).
C
C-     SPICELIB Version 1.2.0, 7-DEC-1990 (MJS)
C
C         Module was updated to include the value of DPMIN for
C         the Macintosh.
C
...
C     PC,     Lahey F77 EM/32 FORTRAN
C     HP-UX 9000/750, FORTRAN/9000 Series 700 computers
C     PC/Linux, Fort77
C
      DPMIN = -1.79769313486231D+308
