...
C
C        Bug fix:  added intialization of variable SNGLPT to support
...
C         point exists.
C  
C     If we let NPOINT be a closest point to POSITN, then the
C     line segment joining POSITN to NPOINT is parallel to
C     the normal to the ellipsoid at NPOINT.  Moreover,
C     suppose we let SEGMENT(P) be the line segment that
C     connects an arbitrary point P with POSITN.  It can
C     be shown that there is only one point P on the
C     ellipsoid in the same octant at POSITN such that the
C     normal at P is parallel to SEGMENT(P)
C  
C  
...
C  
C     To sharpen a solution we use the computed near point, the computed
C     altitude of POSITN and the normal at the near point to approximate
C     POSITN.  The difference between the approximated position of
C     POSITN and the input value of POSITN is called the error vector.
C     To get a sharpened solution we translate the computed near point
C     by the component of the error vector orthogonal to the normal
C     and then find the mathematical near point to our translated
C     solution.
C  
...
C           in this way it is a straight forward problem to find
...
C              ellipsoid. The denominator of the factor we are
C              going to compute is ( AXIS + LAMBDA/AXIS ).
C              Numerically this may be too close to zero for us
C              to actually divide POINT by it.  However, since
C              our solution algorithm for lambda does not depend
C              upon the differentiability of Q, we can simply truncate
C              its individual terms when we are in danger of
C              division overflows.
C
...
C       -------------------------------------
C
C       I suppose you thought you were done at this point.
C       Not necessarily.  If POINT is INSIDE the ellipsoid and
C       happens to lie in the y-z plane, there is a possibility
C       (perhaps even likelihood) that the nearest point on the
C       ellipsoid is NOT in the y-z plane. we must consider
C       this possibility next.
C
...
C          Fortunately these two cases can be handled simultaneously
C          by code.  However, they are most easily understood if treated
C          separately.
C
C          Case 1.  SNGLPT = 2
C
C          The input to the lambda solution POINT lies in the YZ plane.
C          We have already detected one critical point of the
C          distance function to POINT restricted to the ellipsoid.
C          This point also lies in the YZ-plane.  However, when
C          POINT lies on the YZ-plane close to the center of the
C          ellipsoid, there may be a point that is nearest that does
C          not lie in the YZ-plane. Assuming the existence of such a
...
C
C          This assumes of course that a and b are not equal.  If
C          a and b are the same, then the solution we found above
...
C              y is not zero, the deflating ellipsoid must pass
C              through (x,y,z) before it collapses to a segment.)
C
C
C          Case 2.  SNGLPT = 3
C
...
C          critical points in the XZ-plane and YZ plane.  The point
C          in the XZ-plane is the one to examine.  Why?  We are looking
C          for the point on the ellipsoid closest to POINT.  It must
C          lie in one of these two planes. But the ellipse of
C          intersection with the XZ-plane fits inside the ellipse
C          of intersection with the YZ-plane.  Therefore the closest
C          point on the YZ-ellipse must be at a greater distance than
C          the closest point on the XZ-ellipse.  Thus, in solving
C          the equations
C
C
C                   lambda*x
