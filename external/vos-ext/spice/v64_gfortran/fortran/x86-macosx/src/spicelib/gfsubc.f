C$Procedure GFSUBC (GF, subpoint vector coordinate search )
...
C     subpoint position vector satisfies a numerical constraint.
C
C$ Disclaimer
C
...
C     METHOD     I   Name of method type for subpoint calculation
...
C              name.
C
C     METHOD   the string name of the method to use for the subpoint
C              calculation. The accepted values for METHOD:
C
...
C              coordinate of the subpoint vector. The result 
...
C              to satisfy by the selected coordinate of the subpoint
...
C              and ADJUST is set to a positive value, GFSUBC finds 
C              times when the subpoint position vector coordinate is
C              within ADJUST radians/kilometers of the specified 
C              extreme value.
C
...
C              function of the subpoint vector is monotone increasing or
...
C              will be discarded before GFSUBC conducts its
C              search.
C
C$ Detailed_Output
C
...
C         coordinate events in the GEODETIC or PLANETOGRAPHIC 
C         coordinate systems will cause the SPICE(NOTSUPPORTED) error
C         to signal.
C
C$ Files
C
...
C     subpoint position vector coordinate value events. 
...
C     the subpoint position vector satisfies a caller-specified
...
C     of the solution set for any inequality constraint is the set 
C     of points where an equality constraint is met, the solutions of
C     both equality and inequality constraints can be found easily
C     once the monotone windows have been found.
C      
C
C     Step Size
C     =========
...
C     miss solution intervals if the step size is shorter than the
...
C     routine is set by the parameter CNVTOL.
C 
...
C
C     To use a different tolerance value, a lower-level GF routine such
C     as GFEVNT  must be called. Making the tolerance tighter than
C     CNVTOL is unlikely to be useful, since the results are unlikely
C     to be more accurate. Making the tolerance looser will speed up
C     searches somewhat, since a few convergence steps will be omitted.
C     However, in most cases, the step size is likely to have a much
C     greater effect on processing time than would the convergence
C     tolerance.
C
C
...
C      Example(1):
C
C      Find the time during 2007 for which the subpoint position vector
C      of the sun on earth in the IAU_EARTH frame lies within a geodetic
C      latitude-longitude "box" defined as
C
...
C      The sub-observer point latitude varies between approximately 
C      23.44 degrees and -23.44 degrees during the year. The 
C      sub-observer point longitude varies between -180 degrees and 
C      180 degrees in one day.
C
C           PROGRAM GFSUBC_EX1
C           IMPLICIT              NONE
C     
C     C
...
C           DOUBLE PRECISION      RAD    ( 2 )
C           DOUBLE PRECISION      LON    ( 2 ) 
C           DOUBLE PRECISION      LAT    ( 2 )
C           DOUBLE PRECISION      TRGEPC
C           DOUBLE PRECISION      LPOS   ( 3 )
C           DOUBLE PRECISION      RPOS   ( 3 )
...
C     C     Load kernels.
C     C 
C           CALL FURNSH ('/kernels/standard.tm')
C     
...
C     C     latitude- longitude box restriction conditions apply to the 
C     C      subpoint vector.
C     C
...
C     C     then use that result as confinement for the 
C     C     longitude search.
C     C
C           COORD  = 'LATITUDE'
C           REFVAL = 16.D0 * RPD()
C           RELATE = '>'
C     
C           CALL GFSUBC (  TARGET,  FIXREF,
...
C     C     linear behavior so use a step size less than half the period
...
C     C
C                 CALL WNFETD ( RESULT4, I, LEFT, RIGHT  )
C     
C                 CALL TIMOUT ( LEFT, TIMFMT, BEGSTR )
C                 CALL TIMOUT ( RIGHT, TIMFMT, ENDSTR )
C     
C     C 
C     C           Determine the latitude and longitude of the subpoint
C     C           at the event interval boundaries. 
C     C           
C                 CALL SUBPNT ( METHOD, TARGET, LEFT, FIXREF, ABCORR,
C          .                    OBSRVR, LPOS, TRGEPC, SRFVEC )
C                 CALL RECLAT ( LPOS, RAD(1), LON(1), LAT(1) )
C     
C                 CALL SUBPNT ( METHOD, TARGET, RIGHT, FIXREF, ABCORR,
C          .                    OBSRVR, RPOS, TRGEPC, SRFVEC )
C                 CALL RECLAT ( RPOS, RAD(2), LON(2), LAT(2) )
C     
C     
C                 WRITE(*,*) 'From : ',BEGSTR,LAT(1)*DPR(),LON(1)*DPR()
C                 WRITE(*,*) 'To   : ',ENDSTR,LAT(2)*DPR(),LON(2)*DPR()
C                 WRITE(*,*) ' '
C     
C              END DO 
C     
...
C
C   Time intervals                                Lat         Lon 
C
C   From : 2007-MAY-05 06:14:04.637734 (TDB)      16.0543561  86.
C   To   : 2007-MAY-05 06:18:04.621907 (TDB)      16.0551478  85.0000042
C       
C   From : 2007-MAY-06 06:13:59.583483 (TDB)      16.3371472  86.
C   To   : 2007-MAY-06 06:17:59.569239 (TDB)      16.3379265  85.0000042
C          
C   From : 2007-MAY-07 06:13:55.102939 (TDB)      16.6154436  86.
C   To   : 2007-MAY-07 06:17:55.090299 (TDB)      16.6162103  85.0000042
C       
C   From : 2007-MAY-08 06:13:51.202604 (TDB)      16.8891626  86.
C   To   : 2007-MAY-08 06:17:51.191583 (TDB)      16.8899165  85.0000042
C       
C   From : 2007-AUG-06 06:23:17.282927 (TDB)      16.6807174  86.
C   To   : 2007-AUG-06 06:27:17.264009 (TDB)      16.6799616  85.0000042
C       
C   From : 2007-AUG-07 06:23:10.545441 (TDB)      16.4064108  86.
C   To   : 2007-AUG-07 06:27:10.524925 (TDB)      16.4056426  85.0000042
C       
C   From : 2007-AUG-08 06:23:03.233995 (TDB)      16.1276778  86.
C   To   : 2007-AUG-08 06:27:03.211889 (TDB)      16.1268975  85.0000042
C
C$ Restrictions
C
...
C$ Version
C
C-   SPICELIB Version 1.0.1, 22-AUG-2009 (EDW)
C
C       Edited argument descriptions.
C
C      Edit to Example description, replaced "intercept" with
...
C$ Index_Entries
C
C     GF subpoint coordinate search
C
C-&
...
C
C     Set up a call to GFEVNT specific to the subpoint coordinate
C     search.
C
      QPNAMS(1) = 'TARGET'
      QCPARS(1) =  TARGET
