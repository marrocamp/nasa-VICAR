...
C
C        Removed from the header descriptions, all occurences of the 
...
C         signalled.  HANDLE is is undefined in this case.
C
...
C         SPICE(EKFILETABLEFULL) will be signalled.  HANDLE is
...
C         SPICE(EKSEGMENTTABLEFULL) will be signalled.  HANDLE is
...
C         SPICE(EKCOLDESCTABLEFULL) will be signalled.  HANDLE is
C         is undefined in this case.  This routine will attempt to
C         unload the file from the DAS system.
C
...
C         be signalled.  HANDLE is is undefined in this case.  This
...
C         the error SPICE(EKIDTABLEFULL) will be signalled.  HANDLE is
C         is undefined in this case.  This routine will attempt to
C         unload the file from the DAS system.
C
C     10) If the input file does not contain at least one segment, the
C         error SPICE(EKNOSEGMENTS) will be signalled.
C
C$ Files
C
...
C
C                          First, euthanatize its column descriptors.
...
C                 out of its misery.
C
C                 First, euthanatize the segment's column descriptors.
...
C         error SPICE(NOLOADEDFILES) is signalled.
C
...
C         SPICE(TABLENOTLOADED) is signalled.
C
C$ Files
C
...
C         the error SPICE(NOLOADEDFILES) is signalled.
C
...
C         .FALSE., and the error SPICE(INVALIDINDEX) is signalled.
...
C         error SPICE(INVALIDTYPE) is signalled.
C
