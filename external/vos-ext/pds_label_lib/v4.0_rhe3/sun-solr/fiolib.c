...
/** BEGIN **/
    ...
    strcpy (old_terminator, "\n");
    old_term_len = strlen(old_terminator);
    ...
    switch (new_record_type)             
    {
        ...
        case PDS_RF_BINARY      : strcpy (terminator, "");
                                  term_len = 0;
                                  sprintf (mrs, "MRS=%d", new_record_length);
    ...
    /*-----------------------------------------------------------------------*/
    /** Determine the appropriate terminator characters.                    **/
    /*-----------------------------------------------------------------------*/

    switch (old_record_type)
    {
        case PDS_RF_STREAM_CR   : 
        case PDS_RF_FIXED_CR    : strcpy (old_terminator, PDS_CR_STRING);
                                  break;
        case PDS_RF_STREAM_LF   : 
        case PDS_RF_FIXED_LF    : strcpy (old_terminator, PDS_LF_STRING);
                                  break;
        case PDS_RF_RMS_STREAM  : 
        case PDS_RF_STREAM_CRLF : 
        case PDS_RF_FIXED_CRLF  : strcpy (old_terminator, PDS_CRLF_STRING);
                                  break;
        default                 : strcpy (old_terminator, "");
                                  break;
    ...
    else 
    {
        ...
        if (old_record_length <= term_len) old_record_length = 0;
        ...
        {
            Malloc_String(blanks, new_record_length + term_len + 1)
        ...
        while (buffer = fiox_read_line (infile, old_record_length,
                                  old_record_type, &char_count, &term_found))
        {
            ...
            if (new_has_fixed_records)
            {
                if (! binary_conversion)
                    Realloc_String(buffer, old_record_length + new_record_length + 1)
                else
                {
                    Malloc_String(temp, old_record_length + new_record_length + term_len + 1)
            ...
            if (term_found) fprintf (outfile, "%s", terminator);
...
#ifdef MSDOC_TC
        success = fio_copy_file (pds_temp_data_fname, file_name);
#else
 ...
 *    success_flag:                                                   *
 *$Detailed_Description                                               *
 *    The fio_copy_file routine makes a copiy of a file.              *
 *$Limitations                                                        *
...
/** BEGIN **/
            ...
            (old_file_name != NULL) && (Flush_File(new_file_name)))
    {
        ...
        else
        {
            ...
            char_count = fread (buffer, 1, PDS_BUFFLEN, infile);
            while (! done)
            {
 ...
 *    Item                     Shared-Data           Access           *
 *    ------------------------------------------------------------    *
 *    pds_setup_fname          pdsglob.h             read             *
 *$Author_and_Institution                                             *
 *    David P. Bernath                                                *
 *$Version_and_Date                                                   *
 *    1.0   March 13, 1991                                            *
...
/** BEGIN                                                                  **/
/**                                                                        **/
/** IF the setup file cannot be opened THEN                                **/
/**     Inform the user and prepare to return.                             **/
/** ELSE                                                                   **/
/*--------------------------------------------------------------------------*/

    *text = EOS;

    if ((file_ptr = fopen (pds_setup_fname, "r")) == NULL)
    {
        printf (" *** Unable to find the setup file, %s\n", pds_setup_fname);
        printf (" *** You must copy this file from the system library directory\n");
        printf ("     to your home (default login) directory\n");
    } 
    else 
    { 
/*--------------------------------------------------------------------------*/
 ...
 *    MDD   05-01-91   Changed to return FALSE is the file is a       *
 *                     directory.                                     *
 **********************************************************************/
...
/*--------------------------------------------------------------------------*/
           ...
           util_byte_copy (first_str, byte_str, PDS_NUMBER_OF_VAR_BYTES);
...
/*--------------------------------------------------------------------------*/
           ...
           if ((feof (file_ptr) == 0) && (ferror (file_ptr) == 0))
           {
               ...
               util_byte_copy (&first_str[PDS_NUMBER_OF_VAR_BYTES],
                               keyword_str, PDS_SIZE_OF_KEYWORDS + 1);
...
char *file_name;
{
   ...
   if (file_length != -1 && (file_ptr = fopen (file_name, "r")) != NULL) 
   {
      ...
      Malloc_String(file_string, file_length);
   ...
   {
       err_append_message (ERROR, "The file you specified could not be found.");
   }
   /*-----------------------------------------------------------------*/
 ...
 *$Side_Effects                                                       *
 *    This routine writes messages to stdout.                         *
 *$Error_Handling                                                     *
 ...
 *    then the user is informed and the program exits.                *
 *$External_References                                                *
 *    Item                     Shared-Data           Access           *
 *    ------------------------------------------------------------    *
 *    pds_setup_fname          pdsglob.h             update           *
 *    pds_temp_data_fname      pdsglob.h             update           *
 *    pds_temp_label_fname     pdsglob.h             update           *
 *$Author_and_Institution                                             *
 *    David P. Bernath                                                *
...
/** BEGIN **/
    ...
    /*----------------------------------------------------------------------*/
    /** IF the scratch directory cannot be used THEN                       **/
    /**     Inform the user and exit the program.                          **/
    /** ELSE                                                               **/
    /*----------------------------------------------------------------------*/

    if (! sys_check_directory_integrity (PDS_DEFAULT_LOGIN, NULL, TRUE))
    {
        printf ("\n *** WARNING *** WARNING *** WARNING ***\n\n");
        printf (" Unable to use (%s) as the scratch directory.\n", PDS_DEFAULT_LOGIN);
        printf (" Please contact your system manager.\n");
        printf ("\n *** WARNING *** WARNING *** WARNING ***\n\n");
        Exit_System();
    }
    else
    {
        /*------------------------------------------------------------------*/
        /** Construct the filespecs for all toolbox local files.           **/
        /*------------------------------------------------------------------*/

        pds_setup_fname = util_create_file_spec 
                              (PDS_DEFAULT_LOGIN, PDS_TOOLBOX_SETUP_FNAME);
        pds_temp_data_fname = sys_make_temp_fname (PDS_DEFAULT_LOGIN);
        pds_temp_label_fname = sys_make_temp_fname (PDS_DEFAULT_LOGIN);

    /*----------------------------------------------------------------------*/
    /** ENDIF                                                              **/
    /*----------------------------------------------------------------------*/

    }  /*  End:  "if (! sys_check_directory_integrity ( ... else ..."  */
...

{
    ...
    else
    {
        ...
        }  /*  End:  "if (stat (file_name, &statbuf) != 0) ... else ..."  */

        fclose (file_ptr);

    }  /*  End:  "if ((file_ptr = fopen (file_name, ... else ..."  */
...
/** BEGIN **/
    ...
    *term_found = FALSE;
    ...
    term_char = (record_type == PDS_RF_BINARY) ? NULL : '\n';

#ifndef VAX
    /*-----------------------------------------------------------------------*/
    /** IF this is a VAX, OR it's not a VAX and the record type is not      **/
    /**     RMS_VAR THEN                                                    **/
    /*-----------------------------------------------------------------------*/
    ...
    switch (record_type)
    {
        ...
        case PDS_RF_RMS_VAR     : 
        case PDS_RF_RMS_STREAM  : term_char = '\n';
                                  break;

        case PDS_RF_STREAM_CR   : 
        case PDS_RF_FIXED_CR    : term_char = '\r';
                                  break;

        case PDS_RF_BINARY      : term_char = NULL;
                                  break;

        default                 : term_char = '\n';
                                  break;

    }  /*  End:  "switch (record_type) ..."  */
    ...
    if (record_type != PDS_RF_RMS_VAR)
    {
        ...
        for (i=0, len=0, *char_count=0; ((*char_count < PDS_BUFFLEN) &&
    ...
    else
    {
        ...
        if (! feof(file_ptr) && ! ferror(file_ptr))
        {
            ...
            *char_count = conversion.int_part;
            ...
            if (IsOdd(*char_count)) ++*char_count;
            ...
            else
            {
                ...
                Malloc_String(text, *char_count + 2);
