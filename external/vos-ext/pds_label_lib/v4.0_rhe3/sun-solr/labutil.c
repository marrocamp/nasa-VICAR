...
NULL))
    {
        ...
        lu_parse_string (object_ptr, string, FALSE, TRUE);
                 ...
                 value_ptr = NextValue (value_ptr), ++elements)
        {
            ...
            if (value_string != NULL)
            {
                ...
                Realloc_String(value_set, (String_Size(value_set) + 
                                                    String_Size(value_string)));
                if (*value_set != EOS)
                    strcat (value_set, " ");
                strcat (value_set, value_string);
...
/** BEGIN **/
   ...
   if (ODL_units != NULL)
   {
      ...
      temp_str [len++] = '>';
      temp_str [len] = '\0';
      Malloc_String(unit_str, len);
      strcpy (unit_str, temp_str);
   }
   /*------------------------------------------------------------------*/
...
/**********************************************************************
 *$Component                                                          *
 *    LOGICAL lu_parse_string (label_ptr, odl_string, clear_messages, *
 *                             save_messages)                         *
 *$Abstract                                                           *
 ...
 *    clear_messages:                                                 *
 *        The clear_messages variable is a TRUE/FALSE flag that tells *
 *        the routine whether or not to flush the contents of the     *
 *        redirect files and deallocate the global message list.      *
 *    save_messages:                                                  *
 *        The save_messages variable is a TRUE/FALSE flag that tells  *
 *        the routine whether or not to append the contents of the    *
 *        redirect files onto the global message list.                *
 *$Outputs                                                            *
 *    None                                                            *
 *$Returns                                                            *
 *    success_flag:                                                   *
...
LOGICAL lu_parse_string (label_ptr, odl_string, clear_messages, save_messages)
...
char *file_name;
{
   ...
   FILE *file_ptr;
   int z_byte_count = 0;
   int ddid_len = SFDU_DDID_STOP - SFDU_DDID_START + 1;
...
/** BEGIN **/
   ...
   if (top_ddid != NULL && file_name != NULL && 
          label_ptr != NULL && strlen (top_ddid) == ddid_len)
   {
      /*----------------------------------------------------------------*/
      ...
      else
      {
         ...
         {
            fprintf (file_ptr, "CCSD1Z000001%08dNJPL1IF0%s%08d", 
                        z_byte_count, top_ddid, z_byte_count - 20);
            fclose (file_ptr);
         }
         /*-------------------------------------------------------------*/
         /** ELSE                                                      **/
...
/** BEGIN **/
   ...
   if (top_ddid != NULL && file_name != NULL && 
          label_ptr != NULL && bottom_ddid != NULL && 
             strlen (top_ddid) == ddid_len &&
                strlen (bottom_ddid) == ddid_len)
   {
      /*----------------------------------------------------------------*/
      /** IF the output file could not be opened THEN                  **/
      /**    print an error                                            **/
