...
#if defined(SUN_UNIX) || defined(LINUX) || defined(MAC)
	char *pathSep = "/";
	char *newline = "\x0d\n";
#else
	char *pathSep = "\\";
...
#ifdef XVT_DEF
				xvt_dm_post_note("Line %d: Unable to locate required format file: %s", 
					    kwd->line_number, fname);
#else
...
#endif
{
    ...
    if (object != NULL)
    {
        ...
        if (kwd == NULL) 
        {
            ...
            if (kwd == NULL) 
            {
                kwd = (KEYWORD *) OdlFindKwd(object, "ODL_VERSION_ID", 
                                             NULL, 1, ODL_THIS_OBJECT);
            }
        }
...
#endif
{
    ...
    {
		/* Check for a path separator at the end of the directory spec */
		ptr = vol_dir + strlen(vol_dir)-1;
#ifdef MSDOS_TC
		if(strcmp(ptr, "\\") != 0)
		{
			sprintf(path, "%s\\%s", vol_dir, filename);
			ptr = path + strlen(vol_dir) + 1;
		}
#else
		if(strcmp(ptr, "/") != 0)
		{
...
#endif
{
    ...
    if (object != NULL)
    {
        ...
        if (object->pre_comment != NULL)
            if (strlen( object->pre_comment) > 0)
            {
                ...
                for (i=0; i<strlen( object->pre_comment); i++)
					if (isprint(object->pre_comment[i]))
                    {
                        OdlPrintLine(message_fname, m_ptr, newline);
                        break;
                    }
            }    
        ...
        if (object->parent != NULL)
        {
			...
			{
                if (object->class_name == NULL) {
                  sprintf(msgtext, "%sGROUP", blanks);
                  OdlPrintLine(message_fname, m_ptr, msgtext);
                }
                else {
                  sprintf(msgtext, "%sGROUP = %s", blanks, object->class_name);
                  OdlPrintLine(message_fname, m_ptr, msgtext);
                }
			}
    
			if ((object->line_comment != NULL) && (strlen(object->line_comment) > 0)) {
        ...
        if (object->post_comment != NULL)
            ...
            for (i=0; i<strlen( object->post_comment); i++)
                if (isprint(object->post_comment[i]))
                {
                    OdlPrintLine(message_fname, m_ptr, newline);
                    break;
                }
        ...
        if (object->parent != NULL)
        {
			...
			{
                if (object->class_name == NULL)
                  sprintf(msgtext, "%sEND_GROUP", blanks);
                else
                  sprintf(msgtext, "%sEND_GROUP = %s", blanks,
                        object->class_name);
			}
            OdlPrintLine(message_fname, m_ptr, msgtext);
    
			if ((object->end_comment != NULL) && (strlen(object->end_comment) > 0)) {
                OdlPrintLine(message_fname, m_ptr, " ");
    ...
    if (object != NULL)
    {
                 ...
                 keyword = keyword->right_sibling)
        {
            if (keyword->pre_comment != NULL)
                if (strlen( keyword->pre_comment) > 0)
                {
                    ...
                    for (i=0; i< (int) strlen( keyword->pre_comment); i++)
						if (isprint(keyword->pre_comment[i]))
                        {
                            OdlPrintLine(message_fname, m_ptr, newline);
                            break;
                        }
            ...
            if ((keyword->value != NULL) && (! sfdu_only))
			{
				...
				if((msglength + strlen(keyword->value)) > 78)
				{
					char *temp = NULL;
					char delimiters[2] = {' ', ','};
					char line[81]={0};
					...
					char *ptr = NULL;
					char *tempValue = NULL;
                    int spaces_left = 78 - msglength;
					int endOfString = 0;

					CopyString(tempValue, keyword->value);
                    StripUnprintables(tempValue)       /*  removes linefeeds and such  */
                    ReplaceChar(tempValue, '\t', ' ')  /*  turns TABs into blanks */
					ReplaceChar(tempValue, '\r', ' ') 
					ReplaceChar(tempValue, '\n', ' ')
                    StripTrailing(tempValue, ' ')      /*  removes trailing blanks     */
					ptr = tempValue;

					for(;*(ptr) != NULL;)
					{
                        if(spaces_left < strlen(ptr))
						{
							endOfString = spaces_left;
                            strncpy(line, ptr, spaces_left);
						    line[endOfString] = '\0';
						    linebreak = lastIndexOfAny(line, delimiters, 2);
						    if(linebreak != -1)
						    {
                               line[linebreak + 1] = '\0';
                               StripTrailing(line, ' ')
						...
						{
							endOfString = strlen(ptr);
							strcpy(line, ptr);
  					        ptr += strlen(line);
						}

						OdlPrintLine(message_fname, m_ptr, line);
						OdlPrintLine(message_fname, m_ptr, newline);
						if(*(ptr) != NULL)
						{
							/*Indent the next line*/
                            blanks[kwd_name_column + 2] = '\0';
                            OdlPrintLine(message_fname, m_ptr, blanks);
                            blanks[kwd_name_column + 2] = ' ';
                            spaces_left = 78 - (kwd_name_column + 2);
						}
					}
					LemmeGo(tempValue);
				}
			...
			if ((keyword->line_comment != NULL) && (strlen(keyword->line_comment) > 0))
			{
                OdlPrintLine(message_fname, m_ptr, " ");
...
int lastIndexOfAny(char *text, char delimiters[], int size)
{
	int maxindex = -1;
	int i = 0;
	for(i = 0; i < size; i++)
	{
		char *ptr = strrchr(text, delimiters[i]);
		if(ptr != NULL)
		{
            int index = ptr - text;
			if(index > maxindex)
...

{
    ...
    char *tintext = {NULL};
    char *text = {NULL};
	char line_comment [TB_MAXLINE + 1]={0};
	char intext [TB_MAXLINE + 1]={0};
	char msgtext [TB_MAXLINE + 1]={0};
        ...
        while (! end_found)
        {
            ...
            if ((text != NULL) || (*c != '\0'))
            {
                ...
                else
                {
                    ...
                    {
						char *textCopy = NULL;
						CopyString(textCopy, text)
					    StripTrailing(textCopy, ' ')
                        c = (char *) LastChar(textCopy);
                        val_found = (*c != '=');
						LemmeGo(textCopy);
                    }
