...
C     If our files are CKs or PCKs and ``cov'' frame attributes are
C     missing, set them as well.
C
      IF ( CFRNAM(1) .EQ. ' ' ) THEN

         IF ( ARCTYP(1) .EQ. 'DAF/CK'  .OR.
     .        ARCTYP(1) .EQ. 'DAF/PCK'      ) THEN
         ...
         IF (   KERNAM(1) .EQ. ' '             .AND.
     .        ( ARCTYP(2) .EQ. 'DAF/CK'  .OR.
     .          ARCTYP(2) .EQ. 'DAF/PCK'      )     ) THEN
...
C
C     If we got no file(s) on the command line or if this(ese) file(s)
C     is/are not CK or binary PCK, the user must have provided start
C     and stop times on the command line. If not, we stop.
C
C     If one of the two files or the only file is CK or binary PCK, we
C     will try to get coverage window from it and further restrict this
C     window using start and stop times if they were provided.
C
C     If both files are CKs and/or binary PCKs, we will try to get
C     coverage window from each of them, intersect these windows and
C     them and further restrict the resulting window using start and
C     stop times if they were provided.
C
...
C
     ...
     .                 'no files are given on the command line.'     )
         CALL ERRCH  ( '#', BEGKEY                                   )
         CALL ERRCH  ( '#', ENDKEY                                   )
...
C
C     Next check is for one file not CK or binary PCK and missing start
C     or stop.
C
      IF (   KERNAM(1) .EQ. ' '              .AND.
     .     ( ARCTYP(2) .NE. 'DAF/CK' .AND.
     .       ARCTYP(2) .NE. 'DAF/PCK'      ) .AND.
     .     ( TIME(1)   .EQ. ' '      .OR.
     .       TIME(2)   .EQ. ' '            )       ) THEN
     ...
     .                 'Both start time and stop time must be '      //
     .                 'provided using ''#'' and ''#'' keys when '   //
     .                 'the only file given on the command line '    //
     .                 '-- ''#'' -- is not a CK or binary PCK.'      )
         CALL ERRCH  ( '#', BEGKEY                                   )
         CALL ERRCH  ( '#', ENDKEY                                   )
         CALL ERRCH  ( '#', KERNAM(2)                                )
         CALL SIGERR ( 'SPICE(NOTIMEBOUNDS2)'                        )

      END IF

C
C     Next check is for two files not CK or binary PCK and missing
C     start or stop.
C
      IF ( ( ARCTYP(1) .NE. 'DAF/CK' .AND.
     .       ARCTYP(1) .NE. 'DAF/PCK'      ) .AND.
     .     ( ARCTYP(2) .NE. 'DAF/CK' .AND.
     .       ARCTYP(2) .NE. 'DAF/PCK'      ) .AND.
     .     ( TIME(1)   .EQ. ' '      .OR.
     .       TIME(2)   .EQ. ' '            )       ) THEN
     ...
     .                 'Both start time and stop time must be '      //
     .                 'provided using ''#'' and ''#'' keys when '   //
     .                 'both files given on the command line '       //
     .                 '-- ''#'' and ''#'' -- are not CKs or binary '//
     .                 'PCKs.'                                       )
         CALL ERRCH  ( '#', BEGKEY                                   )
         CALL ERRCH  ( '#', ENDKEY                                   )
         CALL ERRCH  ( '#', KERNAM(1)                                )
         CALL ERRCH  ( '#', KERNAM(2)                                )
         CALL SIGERR ( 'SPICE(NOTIMEBOUNDS3)'                        )
...
C     them and do more checks after that. For the first coverage frame
C     we may get coverage either from the first or the only file. For
C     the second coverage frame we will try to get coverage from the
C     second or only file. But first, let's initialize our coverage
C     windows.
C
...
C
C     Try to get coverage for the first coverage frame if
C
...
C
            ...
            IF ( PTR .NE. 0 ) THEN
               COVDSC(1) = 'Coverage for the CK frame ''#'' (class ' //
     .                     'ID ''#'') was determined from CK file '  //
     .                     '''#''.'
               CALL REPMC ( COVDSC(1), '#', CFRNAM(1), COVDSC(1) )
               CALL REPMI ( COVDSC(1), '#', CFRCID(1), COVDSC(1) )
               CALL REPMC ( COVDSC(1), '#', HKRNAM,    COVDSC(1) )
            ELSE
               COVDSC(1) = 'Coverage for the CK frame with class ID '//
     .                     '''#'' was determined from CK file ''#''.'
               CALL REPMI ( COVDSC(1), '#', CFRCID(1), COVDSC(1) )
               CALL REPMC ( COVDSC(1), '#', HKRNAM,    COVDSC(1) )
            END IF
...
C
            CALL NPARSI ( CFRNAM(1), HINT, ERROR, PTR )
            IF ( PTR .NE. 0 ) THEN
               COVDSC(1) = 'Coverage for the PCK frame ''#'' (class '//
     ...
     .                     '''#''.'
               ...
               CALL REPMC ( COVDSC(1), '#', HKRNAM,    COVDSC(1) )
            ELSE
               COVDSC(1) = 'Coverage for the PCK frame with class '//
...
C
C     Get coverage for the second coverage frame if
C
C         the second file is a CK,
C         the frame is a CK frame, and
C         data for frame's class ID is present in the file.
...
C     conversions, clear loaded kernels and reload the second file and
C     all kernels applicable to the second attitude set.
C
      COVDSC(2) = ' '
...
C
            ...
            IF ( PTR .NE. 0 ) THEN
               COVDSC(2) = 'Coverage for the CK frame ''#'' (class ' //
     .                     'ID ''#'') was determined from CK file '  //
     .                     '''#''.'
            ...
            ELSE
               COVDSC(2) = 'Coverage for the CK frame with class ID '//
     .                     '''#'' was determined from CK file ''#''.'
               CALL REPMI ( COVDSC(2), '#', CFRCID(2), COVDSC(2) )
               CALL REPMC ( COVDSC(2), '#', KERNAM(2), COVDSC(2) )
            END IF
...
C
C     Get coverage for the second coverage frame if the second file is
C     is a PCK, the frame is a PCK frame, and data for frame's class ID
C     is present in the file. No need to reload any kernels for PCK
C     coverage look up.
C
      IF ( ARCTYP(2) .EQ. 'DAF/PCK' .AND.
     .     CFRCLS(2) .EQ. 2         .AND.
     .     CFRFND(2)                      ) THEN
...
C
            CALL NPARSI ( CFRNAM(2), HINT, ERROR, PTR )
            IF ( PTR .NE. 0 ) THEN
               COVDSC(2) = 'Coverage for the PCK frame ''#'' (class '//
     .                     'ID ''#'') was determined from PCK file ' //
     .                     '''#''.'
               CALL REPMC ( COVDSC(2), '#', CFRNAM(2), COVDSC(2) )
               CALL REPMI ( COVDSC(2), '#', CFRCID(2), COVDSC(2) )
               CALL REPMC ( COVDSC(2), '#', KERNAM(2), COVDSC(2) )
            ELSE
               COVDSC(2) = 'Coverage for the PCK frame with class '//
     .                     'ID  ''#'' was determined from PCK '    //
     .                     'file ''#''.'
               CALL REPMI ( COVDSC(2), '#', CFRCID(2), COVDSC(2) )
               CALL REPMC ( COVDSC(2), '#', KERNAM(2), COVDSC(2) )
...
C
     ...
     .                 'run to perform comparison for CKs '          //
...
C
     ...
     .                    'obtained by examining the kernel '        //
