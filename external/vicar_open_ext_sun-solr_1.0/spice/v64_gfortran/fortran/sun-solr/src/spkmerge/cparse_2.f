...
C                          
C     
C     In order to specify the minimum and maximum occurance of keywords
...
C        SPICE(NOHEADNODE) is signalled.
C
C$ Files
C
C     None.
...
C                       position in KWNAM. 
C                       
C        NUMMIN  INT   Minimum occurance of chlid keyword in CHLNUM.
C        
C        NUMMAX  INT   Maximum occurance of chlid keyword in CHLNUM.
C      
C$ Examples
C
...
C$ Exceptions
C
C     If the parser is not initialized an error will be signalled. Other
...
C     '\begin_commads' through the end of the file. The following 
C     code fragment illustrates how one perform this task.
C     
C        CALL RDTEXT ('INPUT.CMD', LINE, EOF)
C        
C        DO WHILE (LINE .NE. '\begin_commads')
C        
C           CALL RDTEXT ('INPUT.CMD', LINE, EOF)
C           
C        END DO
...
C              command, but an error will be signalled when we return 
C              to PARSE later.
C     
...
C$ Detailed_Output
C
C     REASON     is the reason a parser error occured. REASON may
