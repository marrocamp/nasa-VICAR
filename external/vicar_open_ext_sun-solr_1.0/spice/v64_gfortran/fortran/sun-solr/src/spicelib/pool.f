...
C$ Declarations
      ...
      INTEGER               MAXVAR
      PARAMETER           ( MAXVAR =   5003 )
 
      INTEGER               MAXVAL
      PARAMETER           ( MAXVAL = 200000 )
 
      INTEGER               MAXLIN
      PARAMETER           ( MAXLIN =   4000 )
...
C$ Abstract
C
C     Remove all variables from the kernel pool.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C     posted for it indicating that it's watched variable has been
C     updated.
C
C$ Examples
C
...
C$Procedure WRPOOL ( Write the values in pool to a specified unit )
 
      ENTRY WRPOOL ( UNIT )
 
C$ Abstract
C
C     Write the values in the pool to the specified unit.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C     VARIABLE  I/O  DESCRIPTION
C     --------  ---  --------------------------------------------------
C     UNIT       I   Logical unit to which the values in the pool will
C                    be written.
C
C$ Detailed_Input
C
C     UNIT       is the logical unit to which the values in the pool
C                will be written.
C
C$ Detailed_Output
C
...
C$ Abstract
C
C     Determine whether or not any of the variables that are to
C     be watched and have AGENT on their distribution list have been
C     updated.
C
C$ Disclaimer
C
...
C                is given to N.
C
C     IVALS      is the array of values associated with NAME.
C                If NAME is not in the pool with numeric type, no
C                values are given to the elements of IVALS.
C
C     FOUND      is TRUE if the variable is in the pool and has numeric
C                type, FALSE if it is not.
C
...
C$ Index_Entries
C
C     Set the value of a character kernel pool variable
C
C-&
...
C$ Index_Entries
C
C     Set the value of a d.p. kernel pool variable
C
C-&
...
C$ Index_Entries
C
C     Set the value of a numeric kernel pool variable
C
C-&
...
C
C     Initialize the kernel pool if necessary.
C
      CALL ZZPINI (  FIRST,
     .               MAXVAR, MAXVAL, MAXLIN,
     .               BEGDAT, BEGTXT,
...
C$ Exceptions
C
C     1) It's not an error to delete an agent that's not in
C        any notification list. This policy allows routines
C        to delete watches without first having to check that
C        the deletion they're requesting is possible.
C
...
C$ Index_Entries
C
C     view kernel pool watcher data structures
C
C-&
