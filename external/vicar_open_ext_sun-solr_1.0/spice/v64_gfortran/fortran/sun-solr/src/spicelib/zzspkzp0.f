C$Procedure ZZSPKZP0 ( S/P Kernel, easy position )
 
      SUBROUTINE ZZSPKZP0 ( TARG, ET, REF, ABCORR, OBS, PTARG, LT )
...
C        ==============
C
C        ZZSPKZP0 begins by computing the geometric position T(ET) of
...
C        for ABCORR, ZZSPKZP0 computes the position of the target body
...
C        selected, ZZSPKZP0 computes the position of the target body T
...
C                  ET = ET0 + (I-1)*STEP
C
C                  CALL ZZSPKZP0 ( TARGET, ET, FRAME, ABCORR, OBSRVR, 
C              .                 POS,    LT                        )
C
C                  WRITE (*,*) 'ET = ', ET
...
C     the SSB.  Then feed that position into ZZSPKPA0 to compute the
...
C
      IF ( EQSTR( ABCORR, 'NONE' ) ) THEN
         ...
         CALL ZZSPKGP0 ( TARG, ET, REF, OBS, PTARG, LT )
...
C        call ZZSPKSB0, ZZSPKPA0 and return.
C
         IF ( TYPE .EQ. INERTL ) THEN
 
            CALL ZZSPKSB0 ( OBS,  ET, REF, SOBS )
            CALL ZZSPKPA0 ( TARG, ET, REF, SOBS, ABCORR, PTARG, LT )
            CALL CHKOUT ( RNAME )
            RETURN
...
C
         CALL ZZSPKSB0 ( OBS,  ET, 'J2000', SOBS )
         CALL ZZSPKPA0 ( TARG, ET, 'J2000', SOBS, ABCORR, POSTN, LT )
         ...
         ELSE IF ( CENTER .EQ. TARG ) THEN
            LTCENT = LT
         ELSE
            CALL ZZSPKPA0 ( CENTER, ET, 'J2000', SOBS, ABCORR, TEMP,
     .                    LTCENT )
         END IF
...
C
         CALL ZZREFCH0 ( FJ2000, REQFRM, ET-LTCENT, XFORM )
