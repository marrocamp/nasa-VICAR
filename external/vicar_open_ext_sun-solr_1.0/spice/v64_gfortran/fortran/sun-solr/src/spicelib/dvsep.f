...
C$ Exceptions
C 
C     1) The error SPICE(NUMERICOVERFLOW) signals if the inputs S1, S2
C        define states with an angular separation rate ~ DPMAX().
C
...
C
C-    SPICELIB Version 1.0.1, 31-MAR-2009 (EDW)
C
C-&
...
C
C     Now calculate the time derivate of the angular separation between
C     S1 and S2.
C
C     The routine needs to guard against both division by zero 
C     and numeric overflow. Before carrying out the division
C     indicated by equation (10), the routine should verify that
C
C     || U1 x U2 || >  fudge factor * | numerator | / DPMAX()
C
C     A fudge factor of 10.D0 should suffice.
C
C     Note that the inequality is strict.
C
...
C
C     Now check for possible overflow.
C
      NUMR  = VDOT( U1(1), U2(4) ) + VDOT( U1(4), U2(1) ) 
      DENOM = VNORM(PCROSS)
            
      SAFE = DENOM .GT. (10.D0 * DABS(NUMR))/DPMAX()
