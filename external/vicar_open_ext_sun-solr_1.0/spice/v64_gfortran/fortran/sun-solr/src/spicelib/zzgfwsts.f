C$Procedure     ZZGFWSTS ( GF window -- Sift the first 
C                        through the second )
...
C$ Index_Entries
C
C     Find intervals of a window contained in an interval of another
C
C-&
...
C
      ...
      IF ( OVFLOW .GT. 0 ) THEN
         ...
         CALL SETMSG ( 'The output schedule does not have '
     .   //            'sufficient memory to contain the result '
     .   //            'of sifting the two given schedules. The '
     .   //            'output schedule requires space for # '
