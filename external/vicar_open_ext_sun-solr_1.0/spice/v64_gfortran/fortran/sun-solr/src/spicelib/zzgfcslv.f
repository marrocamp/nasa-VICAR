...
C     CNVTOL     P   Convergence tolerance for existence window.
...
C
C     CNVTOL     is the convergence tolerance used for determining the
C                existence window for surface intercept computations.
C
...
C
         DO I = 1, WNCARD( CNFINE )
            ...
            CALL ZZGFSOLV ( ZZGFCOEX,  UDSTEP,  UDREFN,  BAIL,
     .                      UDBAIL,    .FALSE., 0.D0,    START,
     .                      FINISH,    CNVTOL,  RPT,     UDREPU,
     .                      WORK(LBCELL,EXWIDX)                  )
