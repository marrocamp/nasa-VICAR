C$Procedure ZZSPKGP0 ( S/P Kernel, geometric position )
 
      SUBROUTINE ZZSPKGP0 ( TARG, ET, REF, OBS, POS, LT )
...
C                 the SPICELIB subroutine ZZREFCH0.
C
...
C$ Particulars
C
C     ZZSPKGP0 computes the geometric position, T(t), of the target
...
C     relative to a common center.  ZZSPKGP0 reads as many segments
C     as necessary, from as many files as necessary, using files
C     that have been loaded by previous calls to SPKLEF (load
C     ephemeris file).
C
C     ZZSPKGP0 is similar to SPKGEO but returns geometric positions
C     only.
C
C$ Examples
C
...
C               ET = BEGIN + I*DELTA
C
C               CALL ZZSPKGP0 ( MOON, ET, 'J2000', EARTH, POS, LT )
C
...
C
C     2) The ephemeris files to be used by ZZSPKGP0 must be loaded
C        by SPKLEF before ZZSPKGP0 is called.
C
...
C
               ...
               ELSE
                  ...
                  CALL ZZREFCH0( TFRAME(CHLEN), TMPFRM,   ET,   PSXFRM )
...
C
            ...
            ELSE
               ...
               CALL ZZREFCH0 ( CFRAME, TMPFRM, ET,   PSXFRM )
...
C
      ...
      DO I = 2, CTPOS-1
         ...
         ELSE
            ...
            CALL ZZREFCH0 ( TFRAME(I),  TFRAME(I+1), ET,  PSXFRM       )
...
C
         ...
         ELSE
            ...
            CALL ZZREFCH0 ( CFRAME, REFID, ET,  PSXFRM )
...
C        ZZREFCH0 to get things into a common frame.
C
      ...
      ELSE
C
C        Use the more general routine ZZREFCH0 to make the
C        transformation.
C
         CALL ZZREFCH0 ( TFRAME(CTPOS),  CFRAME,          ET,  PSXFRM )
