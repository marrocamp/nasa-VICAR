...
C         the next is the correct sign.  The same assumption is made
C         for quaternion derivatives.
C         
C     
...
C
C     Index of SCLK rate in record:
C
      INTEGER               RATIDX
      PARAMETER           ( RATIDX = 4 )
...
C     between successive quaternions.
C
      IF (  ( SUBTYP .EQ. C05TP1 ) .OR. ( SUBTYP .EQ. C05TP3 )  ) THEN
C
C        For these types, only the quaternions themselves need be
C        adjusted.  
C
C        PRVPTR is the index of the "previous" quaternion---the
C        one to which the successor and its negative will be 
C        compared.
C
         PRVPTR = PKTIDX
...
C
     ...
     .          .LT. VDISTG( RECORD(PRVPTR), RECORD(NEWPTR), 4 ) ) THEN
      ...
      ELSE
C
C        For the Hermite types, the quaternions may need to be
C        adjusted; the derivatives are not adjusted.
C
C        PRVPTR is the index of the "previous" quaternion---the
C        one to which the successor and its negative will be 
C        compared.  PRVDER points to the corresponding derivative.
C
         PRVPTR = PKTIDX
         PRVDER = PKTIDX + 4
...
C
C           Scale the rate from radians/tick to radians/second.
C            
