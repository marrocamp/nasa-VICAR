C$Procedure  REPMF  ( Replace marker with formatted d.p. value )
 
      SUBROUTINE REPMF ( IN, MARKER, VALUE, SIGDIG, FORMAT, OUT )
 
C$ Abstract
C
C     Replace a marker in a string with a formatted double precision
C     value.
C
C$ Disclaimer
C
...
C     FORMAT     I   Format: 'E' or 'F'.
C     OUT        O   Output string.
C     MAXLFD     P   Maximum length of a formatted DP number.
C
C$ Detailed_Input
C
...
C
C     FORMAT         is the format in which VALUE is to be represented.
C                    FORMAT may be any of the following:
C
C                       FORMAT  Meaning      Example
C                       ------  -----------  ----------------
C                       E, e    Scientific   3.14159E+03
C                               (exponent)
C                               notation
C
C                       F, f    Fixed-point  3141.59
C                               notation
C
C$ Detailed_Output
C
...
C                    (exponent) or fixed-point notation, depending on
C                    having the value of FORMAT, and having the number
C                    of significant digits specified by SIGDIG.
C                    The representation of VALUE is produced by the
C                    routine DPSTRF; see that routine for details
C                    concerning the representation of double precision
C                    numbers.
C
...
C$ Parameters
C
C     MAXLFD         is the maximum expected length of the text
C                    representation of a formatted double precision
C                    number. 56 characters are sufficient to hold any
C                    result returned by DPSTRF. (See $Restrictions.)
C$ Files
C
C     None.
C
...
C        Then following the call,
C
C           CALL REPMF ( IN, '  XX  ',  -5.2D-9, 3, 'E', OUT )
C
C        OUT is
C
...
C     3. Let
C
C           IN = 'Invalid operation value.  The value was # units.'
C
C        Then following the call,
C
C           CALL REPMF ( IN, '#', 5.0D1, 3, 'F', IN  )
C
C        IN is
C
C           'Invalid operation value.  The value was 50.0 units..'
C
C
C     4. In the above example, if SIGDIG is 1 instead of 3, IN becomes
C
C           'Invalid operation value.  The value was 50 units.'
C
C
C     5. Let
C
...
C        Then following the call,
C
C           CALL REPMF ( IN, '#', 5.0D1, 100, 'E', IN  )
C
C        IN is
C
...
C        returned by DPSTRF; should that routine change, substantial
...
C$ Index_Entries
C
C     replace marker with formatted d.p. value
C
C-&
...
C
      CALL LJUST   ( FORMAT, GDFMT )
      CALL UCASE   ( GDFMT,  GDFMT )
 
      CALL DPSTRF  ( VALUE, SIGDIG, GDFMT, SUBSTR )
 
      IF (  LASTNB(SUBSTR) .NE. 0  ) THEN
 
         CALL ZZREPSUB ( IN,
     .                   MRKPOS,
     .                   MRKPOS + LASTNB(MARKER) - FRSTNB(MARKER),
     .                   SUBSTR( FRSTNB(SUBSTR):LASTNB(SUBSTR) ),
     .                   OUT  )
