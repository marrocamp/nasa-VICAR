...
C        'SPICE(SPKNOTASUBSET)' is signalled.
C
...
C         are, of course, changed to reflect the location of the new
C         segment.
C
C$ Examples
C
C     In the following code fragment, the descriptor for each segment
C     in a source SPK file is examined. For each segment that covers
C     a target interval, the smallest possible subset is extracted into
C     a custom SPK file.
C
