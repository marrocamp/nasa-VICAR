...
C$ Examples
C
C     Never call this subroutine directly. EVER!
C
C$ Restrictions
C
C     None.
...
C
C     Convert the ET seconds past 2000 to the Julian date TDB.
C
      JDTDB = J2000() + ET/SPD()
...
C
C        Solar.  Anyone know what this means?
C
...
C
C     Force the perturbations to be zero at the epoch by setting a
C     set of variables to the values of the perturbations at the
C     epoch ( T = 0 ).  On subsequent calls, these values will be
C     subtracted from the perturbations.
C
      IF ( DOPERT ) THEN
...
C
C        Subtract the epoch perturbations off the values just
C        calculated.
C
         PE     = PE   - PE0
         PINC   = PINC - PINC0
...
C           calculate this term correctly if XNODES passes from less
C           than 2Pi to greater than zero.
C
