...
C$ Abstract
C
C      Return the Euler angles needed to compute the transformation
C      from inertial to body-fixed coordinates for any body in the
C      kernel pool.
C
C$ Disclaimer
C
...
C      W          O   Angle between the x-axis and the prime meridian.
...
C
C      W           is the angle between the x-axis (inertial) and the
C                  prime meridian of the body. W is given in radians.
C
...
C      for the body at the requested time, the angles, W, DELTA
C      and PHI are computed directly from that file.  These angles
C      are then used to compute RA, DEC and W.  The most recently
C      loaded binary PCK file has first priority followed by previously
C      loaded binary PCK files in backward time order.  If no binary
C      PCK file has been loaded, the text P_constants kernel file
C      is used.
C
...
C
C        The offset LAMBDA is a constant for a given body. LAMBDA is
C        needed to distinguish between the latitude and longitude
C        system and the geometric system (where Prime Meridian always
C        intersects the longest axis).
C
C$ Examples
C
