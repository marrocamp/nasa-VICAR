...
C                 'SPICE(UNKNOWNCKMETA)' being signalled.
C
C$ Detailed_Output
C
...
C     of routines such as CKEZ and CKEZAV.
C
C$ Examples
C
...
C     type ('SCLK' or 'SPK').
C
      CALL CMPRSS ( ' ', 1, META, MYMETA )
      CALL LJUST  ( MYMETA,       MYMETA )
      CALL UCASE  ( MYMETA,       MYMETA )
...
C        (Note that every CK ID  has its own agent.)
C
...
C
         CALL SWPOOL ( AGENT (THIS), 1,    LOOKUP(1,THIS)      )
         CALL CVPOOL ( AGENT (THIS),       UPDATE              )
...
C
C        If we didn't find it, we manufacture an ID code based upon
C        the "convention" used for all CKS so far.  However, the
C        convention assumes that the CK ID will be less than -1000
C        if it's not there is no sensible ID to return.  We return
C        zero in that case.
C
