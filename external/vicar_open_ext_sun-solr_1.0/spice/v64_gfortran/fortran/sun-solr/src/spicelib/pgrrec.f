C$Procedure      PGRREC ( Planetographic to rectangular )
...
C$ Detailed_Output
C
C     RECTAN     The rectangular coordinates of the input point.  See
C                the discussion below in the Particulars header section
C                for details.
C
...
C$ Particulars
C
C     Given the planetographic coordinates of a point, this routine
C     returns the body-fixed rectangular coordinates of the point.  The
...
C
C
C     1) Find the rectangular coordinates of the point having Mars
C        planetographic coordinates:
C
C           longitude = 90 degrees west
C           latitude  = 45 degrees north
C           altitude  = 300 km
C
C
C                 PROGRAM EX1
...
C           
C           C
C           C     Do the conversion.  Note that we must provide
C           C     longitude and latitude in radians.
C           C
C                 LON =  90.D0 * RPD()
C                 LAT =  45.D0 * RPD()
C                 ALT =   3.D2
...
C$ Index_Entries
C
C     convert planetographic to rectangular coordinates
C
C-&
...
C
         CALL CMPRSS ( ' ', 0, KVALUE, TMPSTR )
         CALL UCASE  (         TMPSTR, PGRLON )
...
C
C     Now that we have geodetic longitude in hand, convert the geodetic
C     equivalent of the input coordinates to rectangular coordinates.
C
      CALL GEOREC ( GEOLON, LAT, ALT, RE, F, RECTAN )
