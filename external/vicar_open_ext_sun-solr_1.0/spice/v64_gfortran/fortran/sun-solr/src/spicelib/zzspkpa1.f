C$Procedure ZZSPKPA1 ( S/P Kernel, apparent position only )
 
      SUBROUTINE ZZSPKPA1 ( TARG, ET, REF, SOBS, ABCORR, PTARG, LT )
...
C
C
C        3) Obtain an uncorrected position vector derived directly from
...
C     ==============
C
C        ZZSPKPA1 begins by computing the geometric position T(ET) of
...
C        selected, ZZSPKPA1 computes the position of the target body at
...
C        selected, ZZSPKPA1 computes the position of the target body T
...
C$ Examples
C
C     In the following code fragment, SPKSSB and ZZSPKPA1 are used
...
C           CALL ZZSPKPA1 (  IO,    EPOCH, 'J2000', STVGR2,
C       .                 'LT+S', STIO,   LT              )
C
...
C
C     2) The ephemeris files to be used by ZZSPKPA1 must be loaded
C        (normally by the SPICELIB kernel loader FURNSH) before 
C        this routine is called.
C
C     3) Unlike most other SPK position computation routines, this
...
C
C        Based on SPICELIB Version 2.0.1, 20-OCT-2003 (EDW)
C
C-&
...
C     light time.
C
      CALL ZZSPKGP1 ( TARG,  ET, REF, SSB, PTARG, LT )
...
C
      ...
      DO I = 1, MAXITR
         ...
         CALL ZZSPKGP1 ( TARG,   ET + LTSIGN*LT, REF, SSB, PTARG, LT )
         CALL VSUB   ( PTARG,  SOBS,     TPOS )
         CALL VEQU   ( TPOS,   PTARG )
         LT = VNORM  ( PTARG ) / CLIGHT()
