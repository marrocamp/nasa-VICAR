...
C$ Abstract
C
C     This is an umbrella routine for the entry points available
C     for manipulating different reference frames. It should not
...
C     as seen from an observer with idcode OBS.
C
...
C
C        CHARACTER*(32)        NAME1
C        CHARACTER*(32)        NAME2
C
C        INTEGER               FRAME1
C        INTEGER               FRAME2
C        INTEGER               CENT
...
C        frame idcodes have been reserved for use by the DSN.
...
C        All of these frames are centered at the earth (body 399).
C
C        Accompanying documentation is provided in the appendix
C        ``DSN Frames'' of the SPICE document FRAMES.REQ.
C
C-&
...
C
      ...
      CHARACTER*(WDSIZE)    LOOK2
      CHARACTER*(WDSIZE)    LOOKUP
      CHARACTER*(WDSIZE)    NAME   ( NCOUNT )
      CHARACTER*(WDSIZE)    PNAME
 
      INTEGER               CENTER ( NCOUNT )
      INTEGER               CENTRD ( NCOUNT )
      INTEGER               CORDER ( NCOUNT )
      INTEGER               I
      INTEGER               ID
...
C$Procedure NAMFRM (Name to frame)
...
C$ Index_Entries
C
C     Frame name to frame idcode translation
C
C-&
...
C
      IF ( FIRST ) THEN
         ...
         CALL ZZFDAT ( NCOUNT, NAME,   IDCODE, CENTER, TYPE, TYPEID,
...
C
      ...
      ITEM = BSCHOC ( PNAME, NCOUNT, NAME, NORDER )
C
C     If the name is in our list, we can just look up its idcode in
...
C
      ...
      ELSE
C
C        The items wasn't in the list, see if it's in the kernel pool.
C
...
C$Procedure      FRMNAM (Frame to Name)
 
      ENTRY FRMNAM ( FRCODE, FRNAME )
 
C$ Abstract
C
C     Retrieve the name of a reference frame associated with
C     a SPICE id code.
C
C$ Disclaimer
C
...
C
C                 All reference frame names are 32 or fewer characters
C                 in length.  Thus declaring FRNAME to be CHARACTER*(32)
...
C$ Index_Entries
C
C     Frame idcode to frame name translation
C
C-&
...
C
     ...
     .                 NORDER, CORDER, CENTRD )
      ...
      ITEM = BSCHOI ( FRCODE, NCOUNT, IDCODE, CORDER )
...
C     VARIABLE  I/O  DESCRIPTION
C     --------  ---  --------------------------------------------------
C     FRCODE     I   the idcode for some frame
C     CENT       O   the center of the frame
C     CLASS      O   the class (type) of the frame
C     CLSSID     O   the idcode for the frame within its class.
...
C-    SPICELIB Version 4.0.0, 12-SEP-2005 (NJB)
C
C        Entry point FRINFO is no longer error-free. 
C        The following errors are now diagnosed:
C
...
C
C     If the name is in our list, we can just look up its ID code in
C     the parallel array.
C
      IF ( ITEM .GT. 0 ) THEN
      ...
      ELSE
C
C        The frame ID wasn't in the list; see if the frame name
C        is in the kernel pool.
C
         PNAME     = 'FRAME_#_NAME'
         CALL REPMI (  PNAME, '#', FRCODE, PNAME )
...
C      
C           Start by looking up the central body of the frame. The name
C           of the kernel variable for the body could refer to the
C           frame by name or frame ID; the body itself could be
...
C
C           FOUND has been set to indicate whether we found the frame's
C           center.  If we did, CENT has been assigned.
C
...
C$Procedure CIDFRM ( center SPK id  frame )
...
C                 declared as CHARACTER*(32) to ensure that it can
...
C        CHARACTER*(32) will ensure that the name will not be
C        truncated.
C
C$ Files
C
...
C
C           We've got the name:  See if we have this in our handy list
C           of built-in names.
C
            ITEM = BSCHOC ( PNAME, NCOUNT, NAME, NORDER )
...
C           id-code.
C
            ITEM = BSCHOI ( VALUES(1), NCOUNT, CENTER, CENTRD )
...
C
            ...
            ELSE
C
C              Hmmm. Not on the built-in list.  See if it's in the
C              kernel pool somewhere.
C
               PNAME      = 'FRAME_#_NAME'
...
C$Procedure CNMFRM ( Center name to associated frame )
...
C              with this frame.
C
C              Capture the frame name now, since we're going to modify
C              PNAME.
C              
               FRNAME = PNAME
...
C           id-code.
C
            ITEM = BSCHOI ( VALUES(1), NCOUNT, IDCODE, CORDER )
...
C     of centers.
C
      ITEM = BSCHOC ( FRNAME, NCOUNT, NAME, NORDER )
...
C$Procedure CCIFRM ( Class and class ID to associated frame )
...
C                 CLSSID. FRNAME should be declared as CHARACTER*(32)
...
C     with a given frame class and class ID code. The kernel pool is
C     searched first for a matching frame; if no match is found, then
C     the set of built-in frames is searched.
C
...
C
C     First try to look up from the kernel pool the frame associated
C     with the input class and class ID. Since neither of these input
C     values appears in a kernel variable name, we may have to look at
C     all of the frame specifications in the kernel pool. Start out by
C     looking the frame class assignments from any loaded frame
C     specifications.
C
...
C
C     Getting to this point means we didn't find a matching
C     frame specification in the kernel pool. Check the built-in
C     frame list. Unfortunately, this is a linear search.
C
      DO I = 1, NCOUNT
