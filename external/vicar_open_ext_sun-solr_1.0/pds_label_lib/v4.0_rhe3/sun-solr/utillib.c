 ...
 *    occurances of the input character into a single occurance       *
...
/*--------------------------------------------------------------------------*/
    ...
    temp_str = (char *) malloc (maxlen + 2);
    old_str = (char *) malloc (size);
    new_str = (char *) malloc (2 + size + (int) (size/maxlen));
...
/*--------------------------------------------------------------------------*/
    ...
    for (start = old_str; *start != EOS; start = end)
    {
        ...
        strncpy (temp_str, start, len);         /* Copy line to temp str.   */
        temp_str [len] = '\n';                  /* Add newline char.        */
         ...
         
{
    ...
    if (string != NULL)
    {
        ...
        if (c != NULL)
        {
           ...
           len = c - string + 1;
           Malloc_String(save_string, len + 1);
           strncpy (save_string, string, len);
           *(save_string + len) = EOS;
...

{
   ...
   for (temp = string; temp && *temp; temp++)
   {
      if (*temp == '\\')
      {
         ...
         if (*temp == 't')
         {
            util_byte_copy (temp, temp + 1, strlen (temp + 1));
            *--temp  = '\t';
         }
         else if (*temp == 'n')
         {
...
/** BEGIN **/
   ...
   else
   {
      ...
      Malloc_String(new_ptr -> text, String_Size(text));
