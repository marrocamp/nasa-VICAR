...
#   ===============
#
#   Version 5.1.0  Aug 20, 2002 Boris Semenov
#
...
#
#   Version 5.0.1  Jan 29, 2002 Boris Semenov
#
#      Moved "\rm locallib.a" out of the *.pgm loop to allow 
#      correct compilation of multiple .pgm's.     
#
#   Version 5.0.0  Nov. 13, 2001  Nat Bachman
#
#      Modified the script so that the routines moved.f and movei.f
#      are compiled using the -O2 option. 
#
#      Changed a header comment to remove a reference to CSPICE.
#
...
if ( $?TKLINKOPTIONS ) then
   ...
   set TKLINKOPTIONS = "-static-libgfortran -static-libgcc -m32  "
   echo " "
...
if ( $status == 0 ) then
   ...
   foreach SRCFILE ( *.f )
      ...
      if ( ( $SRCFILE == "moved.f" ) || ( $SRCFILE == "movei.f" ) ) then
         $F_COMPILER $F_COMPILEOPTIONS -O2 $SRCFILE
      else
