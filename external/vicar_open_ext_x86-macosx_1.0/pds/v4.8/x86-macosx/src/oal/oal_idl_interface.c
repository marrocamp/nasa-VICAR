  ...
  ***************************************************************************/
  ...
  if        (strcmp( proc_name, "OaParseLabelFile") == 0) {
    ...
    *int4_ptr = (int4) OaRouteErrorMessages( str, fd);
    ...
    int2 = (int) *((int4 *) argv[3]);           /* sample */
    ptr_result = (char *) argv[4];              /* return value */
/*!!!! THE EXTRA ARGUMENT IS PROBABLY WRONG!  Should come from IDL.  */
/*!!!! But that has unknown effects.  Bug exists on all platforms    */
/*!!!! but only Mac complains about it.  rgd 2010/03/04.	     */
    ptr = (char *) OaReadSpectrumFromQube( odltree, int1, int2, 1);
    memcpy( ptr_result, &ptr, sizeof( char *));
    return( "OK");
